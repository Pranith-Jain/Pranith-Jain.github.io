...
async function ensureSeed(db) {
  ...
  const stmt = db.prepare(`INSERT OR IGNORE INTO promptvault_entries
...
export async function promptVaultList(env, opts = {}) {
  ...
  const res = await db
    ...
    .bind(...binds)
    .all();
  return (res.results ?? []).map((r) => rowToEntry(r));
}
export async function promptVaultGet(env, slug) {
  const db = env.BRIEFINGS_DB;
  if (!db) throw new Error('BRIEFINGS_DB D1 binding missing');
  await ensureSchema(db);
  await ensureSeed(db);
...
export async function promptVaultCreate(env, input) {
  const db = env.BRIEFINGS_DB;
  if (!db) throw new Error('BRIEFINGS_DB D1 binding missing');
...
export async function promptVaultRate(env, input) {
  const db = env.BRIEFINGS_DB;
  if (!db) throw new Error('BRIEFINGS_DB D1 binding missing');
...
export function promptVaultCategories() {
