 ...
 */
const ALLOWED_SHIFTS = ['morning', 'afternoon', 'night', 'weekend', 'oncall'];
// ---------------------------------------------------------------------------
...
// CRUD operations.
// ---------------------------------------------------------------------------
export async function shiftlogCreate(env, input) {
...
export async function shiftlogGet(env, id) {
  const db = env.BRIEFINGS_DB;
...
export async function shiftlogList(env, opts = {}) {
  const db = env.BRIEFINGS_DB;
  if (!db) throw new Error('BRIEFINGS_DB D1 binding missing');
  await ensureSchema(db);
...
export async function shiftlogUpdate(env, id, patch) {
  const db = env.BRIEFINGS_DB;
  if (!db) throw new Error('BRIEFINGS_DB D1 binding missing');
  await ensureSchema(db);
...
export async function shiftlogClose(env, id, endedAt) {
