...
const CORPUS: Hypothesis[] = [
  {
    ...
    narrative: 'Attacker deploys an AitM phishing kit (e.g., Evilginx, Muraena) between user and a real IdP login page. Captures session cookie in real time, bypassing MFA. User believes they logged in normally; attacker uses the stolen session cookie to access mailbox, OneDrive, and downstream SaaS apps.',
    ...
    sampleKql: 'SigninLogs | where UserPrincipalName == "user@contoso.com" | where CreatedDateTime > ago(7d) | summarize dcount(IPAddress), make_set(IPAddress) by bin(CreatedDateTime, 1h)',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker uses mshta.exe to fetch and execute a remote .hta file. The HTA runs under the user context with no integrity-level elevation but inherits the user\'s network access and can launch further payloads (PowerShell, .NET assemblies).',
    ...
    sampleKql: 'DeviceProcessEvents | where FileName =~ "mshta.exe" | where ProcessCommandLine has_any ("http://","https://","hxxp","hxxps") | project Timestamp, DeviceName, AccountName, ProcessCommandLine, InitiatingProcessFileName',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker uses a small list of common passwords ("Spray2024!", "Welcome1") against a large list of user accounts from a distributed botnet. Defeats account-lockout policies because each account sees only one attempt.',
    ...
    sampleKql: 'SigninLogs | where ResultType in ("50126","50053") | summarize Failures=count(), Users=dcount(UserPrincipalName) by IPAddress, bin(CreatedDateTime, 1h) | where Failures > 20 and Users > 5',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker registers a multi-tenant Azure AD application, abuses Verified Publisher / publisher verification, and sends a phishing email with a link to the consent page. User grants Mail.Read, Files.ReadWrite.All, etc. Attacker reads mail exfiltrating data and sends phishing from inside the tenant.',
    killChainPhase: 'initial-access',
    ...
    sampleKql: 'AuditLogs | where OperationName in ("Consent to application","Add OAuth2PermissionGrant") | where TargetResources[0].displayName has_any ("Mail.ReadWrite","Files.ReadWrite.All","Directory.ReadWrite.All") | project TimeGenerated, UserId, TargetResources',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker loads a .NET assembly into PowerShell via [System.Reflection.Assembly]::Load() or System.Management.Automation to avoid writing a .dll to disk. Often observed with EncodedCommand, AMSI bypass attempts, or amsiInitFailed strings.',
    ...
    sampleKql: 'DeviceProcessEvents | where FileName =~ "powershell.exe" | where ProcessCommandLine has "EncodedCommand" or ProcessCommandLine has "amsiInitFailed" or ProcessCommandLine has "FromBase64String" | project Timestamp, DeviceName, AccountName, ProcessCommandLine',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'In the 4–24h before encryption, ransomware crews delete shadow copies, clear event logs, and disable recovery. Watch for vssadmin, wbadmin, bcdedit, wevtutil clearing logs, and mass file renames in user-writable directories.',
    ...
    sampleKql: 'DeviceProcessEvents | where FileName in~ ("vssadmin.exe","wbadmin.exe","bcdedit.exe","wevtutil.exe") | where ProcessCommandLine has_any ("delete","clear","recoveryenabled No") | project Timestamp, DeviceName, AccountName, ProcessCommandLine',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker compromises a developer endpoint (phishing, malvertising) and exfiltrates long-lived cloud credentials (AWS access keys, GitHub PATs, npm tokens, container registry creds) from environment variables, .aws/credentials, or browser session stores.',
    killChainPhase: 'credential-access',
    ...
    sampleKql: 'DeviceProcessEvents | where FileName in~ ("aws","kubectl","gh","docker") | where InitiatingProcessFileName !in~ ("bash.exe","cmd.exe","WindowsTerminal.exe","code.exe","Cursor.exe","pwsh.exe") | project Timestamp, DeviceName, ProcessCommandLine, InitiatingProcessFileName',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker encodes payload into DNS query labels (base32/base64 chunks sent as subdomains of attacker.com). The DNS resolver forwards queries to attacker-controlled authoritative NS, which reassembles data. Common for initial C2 and slow exfil.',
    ...
    sampleKql: 'DnsEvents | where TimeGenerated > ago(24h) | summarize Queries=count(), dNames=dcount(Name) by DeviceName | where dNames > 5000',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker publishes a typosquatted or hijacked package (e.g., event-stream, ua-parser-js, ctx) that runs a postinstall script stealing env vars, SSH keys, and CI tokens.',
    ...
    sampleKql: 'union isfuzzy=true (DeviceProcessEvents, DeviceNetworkEvents) | where InitiatingProcessFileName in~ ("npm","node","pip","python","pip3") | where RemoteUrl has_any (".tk",".ml",".ga",".cf",".xyz") or RemoteIP in ("<low-reputation-IPs>")',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker with code execution on a host dumps LSASS process memory to harvest NTLM hashes, Kerberos tickets, and cleartext credentials (where WDigest is enabled). Common via Mimikatz, ProcDump, comsvcs.dll MiniDump, or direct handle to lsass.exe.',
    killChainPhase: 'credential-access',
    ...
    sampleKql: 'DeviceProcessEvents | where FileName in~ ("procdump.exe","rundll32.exe") | where ProcessCommandLine has "lsass" or ProcessCommandLine has "MiniDump" | project Timestamp, DeviceName, AccountName, ProcessCommandLine',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Encrypted-workstation or file-server traffic to a known ransomware tracker (.onion gateway egress, payment portal domain, or known I2P endpoint) within hours of mass file rename events.',
    killChainPhase: 'command-and-control',
    ...
    sampleKql: 'DeviceNetworkEvents | where RemotePort in (443,8443) | where RemoteIP in (TorExitNodeIPs) | join kind=inner DeviceProcessEvents on DeviceName, $left.Timestamp between ($right.Timestamp .. ($right.Timestamp + 30m))',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker encodes stolen data into DNS TXT record queries. Defender\'s DNS logs show a flood of TXT queries to attacker.com containing base64-chunked payloads.',
    ...
    sampleKql: 'DnsEvents | where QueryType == 16 | where Name matches regex @"^[A-Za-z0-9+/=]{20,}\\." | summarize count() by Name, bin(TimeGenerated, 5m)',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker obtains a user\'s account (via leaked password, social engineering) and resets MFA. The account has been dormant for 90+ days; activation triggers a sign-in from an unusual IP. The risk event in Azure AD Identity Protection may have been suppressed.',
    killChainPhase: 'initial-access',
    mitre: ['T1078', 'T1098'],
    signals: [
    ...
    sampleKql: 'SigninLogs | where ResultType == 0 | extend LastSignIn = coalesce(toscalar(SigninLogs | where UserPrincipalName == UserPrincipalName and TimeGenerated < ago(1d) | summarize max(TimeGenerated)), datetime(1900-01-01)) | extend DaysSince = datetime_diff("day", TimeGenerated, LastSignIn) | where DaysSince > 90',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker with privileged access to AD or Azure AD connector clears Security event logs to hide their activity. Watch for wevtutil cl, Get-EventLog -Clear, or log-pipeline tampering (omitting event-id 4624, 4720, etc.)',
    killChainPhase: 'defense-evasion',
    ...
    sampleKql: 'DeviceProcessEvents | where FileName =~ "wevtutil.exe" | where ProcessCommandLine has "cl " | project Timestamp, DeviceName, AccountName, ProcessCommandLine',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker compromises a low-privilege service account, finds it has GenericAll / WriteDACL / DCSync rights, escalates to Domain Admin. Watch for service accounts with privileged group membership, recent ACL changes, and Kerberos pre-auth disabled.',
    ...
    sampleKql: 'AuditLogs | where OperationName in ("Add member to group","Add member to role") | where TargetResources[0].userPrincipalName has "svc-" or TargetResources[0].userPrincipalName has "sa-" | project TimeGenerated, Actor, TargetResources',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'User opens a weaponised .docm / .xlsm with a malicious VBA macro. The macro runs on Document_Open, spawns wscript.exe or powershell.exe, fetches a stager, executes in memory. Common initial access vector for ransomware affiliates.',
    killChainPhase: 'execution',
    ...
    sampleKql: 'DeviceProcessEvents | where InitiatingProcessFileName in~ ("winword.exe","excel.exe","outlook.exe","powerpnt.exe") | where FileName in~ ("powershell.exe","wscript.exe","cscript.exe","mshta.exe") | project Timestamp, DeviceName, ProcessCommandLine, InitiatingProcessFileName',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker with a low-privilege cloud identity (often an SSO user with weak RBAC) creates a new access key, attaches AdministratorAccess, or modifies a Lambda execution role to gain cross-account access.',
    killChainPhase: 'privilege-escalation',
    ...
    sampleKql: 'AWSCloudTrail | where EventName in ("AttachUserPolicy","CreateAccessKey","UpdateAssumeRolePolicy") | where UserIdentity.type == "IAMUser" or UserIdentity.type == "AssumedRole" | project TimeGenerated, UserIdentity, EventName, RequestParameters',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker with a foothold scans the internal network to discover other hosts, services, and credentials. Common signatures: powershell Test-NetConnection at scale, SMB session enumeration, LDAP queries for service accounts.',
    ...
    sampleKql: 'DeviceProcessEvents | where FileName in~ ("nmap.exe","masscan.exe") or ProcessCommandLine has "Test-NetConnection" | project Timestamp, DeviceName, AccountName, ProcessCommandLine',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker steals a long-lived OAuth refresh token (from local browser session storage, GitHub repo leak, CI/CD pipeline env var) and uses it offline to query M365 Graph, Google Workspace, Salesforce.',
    killChainPhase: 'credential-access',
    ...
    sampleKql: 'MailItemsAccessed | where ClientAppName != "Outlook Desktop" | summarize Calls=count() by MailboxOwner, ClientAppName, bin(TimeGenerated, 1h) | where Calls > 100',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker or insider uses a personal Dropbox/Google Drive/MEGA account to upload sensitive files (source code, customer PII, financial models) bypassing corporate DLP.',
    killChainPhase: 'exfiltration',
    ...
    sampleKql: 'DeviceFileEvents | where SensitivityLabel has "Confidential" | where ActionType in ("FileOpened","FileRenamed") | join kind=inner DeviceNetworkEvents on DeviceName, $left.Timestamp between ($right.Timestamp - 5m .. $right.Timestamp + 5m) | where RemoteUrl has_any ("dropbox.com","drive.google.com","mega.nz")',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Compromised host makes DNS queries to algorithmically-generated domain names (NXDOMAIN-heavy traffic, high-entropy labels) used by malware for resilient C2.',
    killChainPhase: 'command-and-control',
    ...
    sampleKql: 'DnsEvents | where ResponseCode == 3 | summarize NX=count() by DeviceName, bin(TimeGenerated, 1h) | where NX > 100',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Former employee / contractor retains access to corporate SaaS via personal OAuth grant, unscoped service account, or unrevoked API key. Used for revenge or data theft.',
    ...
    sampleKql: 'SigninLogs | where TimeGenerated > ago(30d) | join kind=leftouter (AuditLogs | where OperationName == "Disable account") on UserPrincipalName | where ResultType == 0 and AuditTimeGenerated < TimeGenerated',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'User enters password on a phishing clone of the M365 login page. Attacker uses the credentials in real time, often with IMAP/ActiveSync first (lower bar) before moving to interactive logins.',
    killChainPhase: 'credential-access',
    mitre: ['T1078', 'T1110'],
    signals: [
    ...
    sampleKql: 'SigninLogs | where ClientAppUsed in ("IMAP4","ActiveSync","POP3","SMTP") | where ResultType == 0 | summarize dcount(IPAddress) by UserPrincipalName | where dcount_IPAddress > 3',
    score: 0,
    rationale: '',
  },
  {
    ...
    narrative: 'Attacker compromises a pod running with privileged: true or hostPath mount, escapes to node, and pivots to the cluster. Watch for kubectl exec from unusual sources, privileged pod creation, and hostPath mounts.',
    killChainPhase: 'privilege-escalation',
    mitre: ['T1611', 'T1078'],
    signals: [
    ...
    sampleKql: 'KubeAudit | where ObjectRef.resource == "pods" and RequestObject.spec.containers[0].securityContext.privileged == true',
    score: 0,
    rationale: '',
  },
];
...
const STOPWORDS = new Set([
  'the','a','an','and','or','of','to','in','on','for','with','at','by','is','it','this','that','are','be','as','from','user','host','device','process','file','data','event','alert','activity','signs','flag','look','find'
]);
...
function scoreHypothesis(h: Hypothesis, obs: HypoObservation): { score: number; rationale: string } {
  ...
  const hTokens = new Set(tokenize(h.title + ' ' + h.narrative + ' ' + h.signals.join(' ') + ' ' + h.whatToLookFor.join(' ')));
  let overlap = 0;
