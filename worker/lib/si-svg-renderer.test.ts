...
const palette = {
  primary: '#58a6ff', secondary: '#a371f7', accent: '#d2a8ff',
  danger: '#f85149', warning: '#d29922', success: '#3fb950',
  text: '#e6edf3', background: '#0d1117',
};
...
describe('renderDashboard — full widget coverage', () => {
  it('title-banner: emits title + subtitle text', () => {
    const svg = renderDashboard({
      canvas: baseCanvas, palette, widgets: [
        { type: 'title-banner', name: 'header', title: 'Threat Pulse 2026-06-13', subtitle: 'Daily scan' },
      ],
    }, {});
  ...
  it('kpi-card: renders the value', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'kpi-card', name: 'k1', label: 'Open Incidents', value: 12 },
    ]}, {});
  ...
  it('delta-kpi-card: renders value + delta', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'delta-kpi-card', name: 'k1', label: 'New IOCs', value: 47, delta: -3, comparison_period: 'vs. yesterday' },
    ]}, {});
  ...
  it('score-card: renders 0-100 score', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'score-card', name: 's1', label: 'Risk Score', value: 78 },
    ]}, {});
  ...
  it('donut-chart: renders the segment legend with each label', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'donut-chart', name: 'd1', segments: [
        { label: 'Critical', value: 3 }, { label: 'High', value: 7 }, { label: 'Medium', value: 12 },
      ]},
    ]}, {});
  ...
  it('stacked-bar-chart: renders category labels', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'stacked-bar-chart', name: 'b1', categories: [
        { label: 'Cloud', values: [{ label: 'Critical', value: 4 }, { label: 'High', value: 8 }] },
        { label: 'Identity', values: [{ label: 'Critical', value: 1 }, { label: 'High', value: 3 }] },
      ]},
    ]}, {});
  ...
  it('horizontal-bar-chart: renders row labels', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'horizontal-bar-chart', name: 'h1', bars: [
  ...
  it('line-chart: renders the time series polyline', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'line-chart', name: 'l1', points: [
        { x: '2026-06-12', y: 10 }, { x: '2026-06-13', y: 22 },
      ]},
    ]}, {});
  ...
  it('waterfall-chart: renders step labels', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'waterfall-chart', name: 'w1', steps: [
  ...
  it('sparkline: renders a label + line element', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'sparkline', name: 'sp1', label: 'Trend', values: [1, 3, 2, 5, 4, 6, 8] },
    ]}, {});
  ...
  it('progress-bar: renders the label', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'progress-bar', name: 'p1', label: 'Coverage', value: 78, max: 100, sublabel: '78%' },
    ]}, {});
  ...
  it('table-widget: renders column headers + row cells', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'table-widget', name: 't1',
        columns: [{ key: 'value', label: 'Indicator' }, { key: 'count', label: 'Reports' }],
        rows: [{ value: '203.0.113.42', count: 12 }, { value: '198.51.100.10', count: 5 }] },
    ]}, {});
  ...
  it('recommendation-cards: renders card titles', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'recommendation-cards', name: 'r1', recommendations: [
  ...
  it('assessment-banner: renders the assessment text', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'assessment-banner', name: 'a1', title: 'Verdict', assessment: 'Elevated', description: 'Sustained AiTM activity against finance staff.' },
    ]}, {});
  ...
  it('coverage-matrix: renders row + column labels', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'coverage-matrix', name: 'c1', field: [
        { column: 'Email', items: [{ name: 'T1566 Phishing', status: 'covered' }, { name: 'T1078', status: 'partial' }] },
        { column: 'Identity', items: [{ name: 'T1566 Phishing', status: 'partial' }, { name: 'T1078', status: 'covered' }] },
        { column: 'Endpoint', items: [{ name: 'T1566 Phishing', status: 'uncovered' }, { name: 'T1078', status: 'partial' }] },
      ]},
    ]}, {});
  ...
  it('unknown widget type falls back to a dashed warning panel', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      { type: 'not-a-real-widget', name: 'x', label: '?' },
    ]}, {});
  ...
  it('combined manifest with 3 widgets renders them all', () => {
    const svg = renderDashboard({ canvas: baseCanvas, palette, widgets: [
      ...
      { type: 'donut-chart', name: 'd', segments: [{ label: 'High', value: 5 }, { label: 'Med', value: 3 }] },
    ]}, {});
