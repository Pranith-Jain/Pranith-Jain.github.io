...
export function foldHomographs(s) {
  let out = '';
  let changed = false;
  for (const ch of s) {
...
function runWithMeta(input, pattern, captureGroup = 0) {
  const hits = [];
  pattern.lastIndex = 0;
  let m;
...
const NETWORK = ['ipv4', 'ipv6', 'domain', 'url', 'email', 'mac', 'asn'];
const HOST = ['md5', 'sha1', 'sha256', 'sha512', 'registry', 'process', 'dll', 'filePath', 'port'];
const THREAT = ['cve', 'mitre'];
const ALL_KINDS = [...NETWORK, ...HOST, ...THREAT];
export function parseArtifacts(input, opts = {}) {
  ...
  for (const long of ['sha256', 'sha1', 'md5']) {
  ...
  const iocs = { network: [], host: [], threat: [] };
...
export function siParseText(input, opts = {}) {
