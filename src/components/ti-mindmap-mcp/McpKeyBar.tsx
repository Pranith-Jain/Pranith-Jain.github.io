...
export function McpKeyBar({ className = '' }: { className?: string }): JSX.Element {
    ...
    if (status === 'connected') {
      return (
        <>
          ...
          <span className="hidden sm:inline">MCP · 25 tools</span>
          <span className="sm:hidden">MCP</span>
        </>
      );
    }
  ...
  const tone = (() => {
    ...
    if (status === 'unconfigured')
      return 'border-slate-300 dark:border-[rgb(var(--border-400))] bg-slate-50 dark:bg-[rgb(var(--surface-200))] text-body';
    return 'border-slate-300 dark:border-[rgb(var(--border-400))] bg-slate-50 dark:bg-[rgb(var(--surface-200))] text-body';
  })();
        ...
        <div className="absolute right-0 mt-1.5 w-80 max-w-[calc(100vw-1rem)] surface-card shadow-e2 p-3 z-50">
          ...
          <div className="mt-2 flex items-start justify-between gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded border border-slate-300 dark:border-[rgb(var(--border-400))] px-1.5 py-0.5 text-micro font-mono text-body hover:border-brand-400 disabled:opacity-50 shrink-0"
  ...
  return (
    ...
    >
      <input
        ...
        className="rounded border border-slate-300 dark:border-[rgb(var(--border-400))] bg-white dark:bg-[rgb(var(--input-200))] px-2 py-1 font-mono text-xs text-heading w-full sm:w-56"
        autoComplete="off"
        spellCheck={false}
      />
