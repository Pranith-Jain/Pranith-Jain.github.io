...
export function BackLink({ to, className, children, ...rest }: BackLinkProps): JSX.Element | null {
  ...
  return (
    <Link
      ...
      className={
        className ??
        'inline-flex items-center gap-1.5 px-3 py-1.5 -ml-3 text-tool text-muted hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-[rgb(var(--hover-100))] rounded-xl mb-8 font-mono transition-colors'
      }
      {...rest}
    >
      <ArrowLeft size={14} />
