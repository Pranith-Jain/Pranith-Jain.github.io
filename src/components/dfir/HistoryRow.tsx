...
export function HistoryRow({ e }: { e: HistoryEntry }): JSX.Element {
  const verdictType = (['clean', 'suspicious', 'malicious'] as const).includes(e.verdict as never)
    ...
    : 'unknown';
  return (
    <li className="flex items-center justify-between rounded-xl border border-slate-200 dark:border-[rgb(var(--border-400))] bg-white dark:bg-[rgb(var(--surface-200))] shadow-e1 p-3">
