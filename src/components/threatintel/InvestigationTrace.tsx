...
export function InvestigationTrace({ steps }: { steps: TraceStep[] }): JSX.Element {
  ...
  const totalTools = steps.reduce((n, s) => n + s.toolCalls.length, 0);
                ...
                {isStepExpanded && (
                  <div className="border-t border-slate-100 dark:border-[rgb(var(--border-400))] px-2.5 py-2 space-y-2">
                    ...
                    <div className="text-xs text-slate-600 dark:text-slate-400">
                      <span className="font-mono text-slate-400">plan:</span> {step.plan}
                    </div>
                      ...
                      <div key={i} className="flex items-start gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            {step.results?.[i] && (
                              <span className={`text-xs ${step.results[i].status === 'ok' ? 'text-emerald-500' : 'text-rose-500'}`}>
                                {step.results[i].status === 'ok' ? '✓' : '✗'}
                                {step.results[i].durationMs ? ` ${step.results[i].durationMs}ms` : ''}
                              </span>
                            )}
                          </div>
                          {tc.reasoning && (
                          ...
                          {step.results?.[i]?.error && (
                            <div className="mt-0.5 text-xs text-rose-500">
                              error: {step.results[i].error}
                            </div>
                          )}
                        </div>
                      </div>
