      ...
      <div className="p-4 space-y-3">
        {loading && !intel && (
          <div className="flex items-center gap-2 justify-center py-6">
            ...
            <span className="text-xs text-muted">Generating intelligence brief…</span>
          </div>
        )}
        ...
        {intel && (
          <>
            <div className="flex items-center gap-2">
              ...
              <span
                className={`inline-flex items-center gap-1 text-micro font-mono ${intel.trend === 'deteriorating' ? 'text-rose-400' : intel.trend === 'improving' ? 'text-emerald-400' : 'text-muted'}`}
              >
                <TrendIcon size={12} />
                {intel.trend}
              </span>
            ...
            {intel.cyber_threats && (
              <div className="rounded-xl bg-rose-500/5 border border-rose-500/10 p-3">
                ...
                <p className="text-xs text-muted">{intel.cyber_threats}</p>
              </div>
            )}
            ...
            {intel.geopolitical_risks && (
              <div className="rounded-xl bg-amber-500/5 border border-amber-500/10 p-3">
                ...
                <p className="text-xs text-muted">{intel.geopolitical_risks}</p>
              </div>
            )}
            ...
            {intel.recommended_posture && (
              <div className="rounded-xl bg-emerald-500/5 border border-emerald-500/10 p-3">
                ...
                <p className="text-xs text-muted">{intel.recommended_posture}</p>
              </div>
            )}

            <CountryShareRow intel={intel} country={country} />
          </>
        )}
      </div>
    </div>
...
function CountryShareRow({ intel, country }: { intel: CountryIntel; country: string }) {
  const pageUrl = typeof window !== 'undefined' ? window.location.href : '';
  const shareText =
    ((intel as unknown as Record<string, unknown>).tweet as string) ||
    intel.executive_summary ||
    `${country} threat level: ${intel.overall_threat_level} #ThreatIntel #CyberSecurity`;
  return (
