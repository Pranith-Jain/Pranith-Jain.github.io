        ...
        {options.map((opt) => {
          ...
          return (
            <label
              ...
              htmlFor={id}
              className={[
                'flex cursor-pointer items-center gap-2 rounded border px-3 py-2 text-sm font-mono transition-colors',
            ...
            >
              ...
              <span>{opt.label}</span>
              {opt.description && <span className="text-micro text-slate-500 dark:text-slate-400">· {opt.description}</span>}
            </label>
          );
        })}
      </div>
