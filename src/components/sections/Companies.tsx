    ...
    <section id="companies" className="scroll-mt-24">
      ...
      <div className="flex flex-wrap gap-2.5">
        {companies.map((company) => (
          <div
            key={company}
            className="rounded-xl border border-slate-200 dark:border-[rgb(var(--border-400))] bg-white dark:bg-[rgb(var(--surface-200)/0.4)] px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 transition hover:border-brand-500/40"
          >
            {company}
          </div>
        ))}
