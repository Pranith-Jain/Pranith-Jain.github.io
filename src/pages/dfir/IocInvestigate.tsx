...
const TABS: Array<{ id: TabId; label: string; desc: string }> = [
  ...
  { id: 'graph', label: 'Pivot Graph', desc: 'Same 26-source checker rendered as radial graph with clickable pivot nodes' },
...
export default function IocInvestigate(): JSX.Element {
  ...
  return (
    ...
    >
      <nav className="flex flex-wrap gap-1 border-b border-slate-200 dark:border-slate-800 mb-6" aria-label="IOC investigation">
        {TABS.map((t) => (
          <button
            key={t.id}
            type="button"
