...
export default function ImageFingerprint(): JSX.Element {
  ...
  const Slot = ({ n, set, r }: { n: number; set: (v: { a: string; d: string }) => void; r: typeof r1 }) => (
    <div className="rounded-xl border border-slate-200 dark:border-[rgb(var(--border-400))] bg-white dark:bg-[rgb(var(--surface-200))] shadow-e1 p-3">
      ...
      {r && (
        <div className="font-mono text-mini text-muted space-y-0.5 break-all">
          <div>aHash: {r.a}</div>
          <div>dHash: {r.d}</div>
        </div>
      )}
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-8 py-6 text-slate-900 dark:text-slate-100">
      ...
      {cmp && (
        <div className="mt-4 rounded-xl border border-slate-200 dark:border-[rgb(var(--border-400))] bg-white dark:bg-[rgb(var(--surface-200))] shadow-e1 p-4">
