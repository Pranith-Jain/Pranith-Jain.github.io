...
/**
 * The DFIR / security-toolkit catalog.
 *
 * Mirrors `pages/threatintel/Catalog.tsx`. Single source of truth for
 * "where can I go in /dfir/*?" - every routable page in the DFIR area,
 * grouped by hub, with a search box and category filter.
 *
 * URL params:
 ...
 */
export default function DfirCatalogPage(): JSX.Element {
  ...
  return (
    <>
      ...
      <DataPageLayout
        ...
        description={
          <>
            Every routable page in the DFIR / security-toolkit area - {totalEntries} pages across {CATALOG.length} hubs.
            Search by name, route, or keyword, or filter by category. New pages are added to{' '}
            <Link to="/dfir" className="text-brand-600 underline-offset-2 hover:underline">
              the home page
            </Link>{' '}
            and{' '}
            <Link to="/dfir" className="text-brand-600 underline-offset-2 hover:underline">
...
function CatalogCard({ entry, hubIcon }: { entry: HubPage; hubIcon: LucideIcon }): JSX.Element {
  ...
  return (
    <li>
      <Link
        to={entry.path}
        className="group block h-full surface-card p-3 transition-[transform,border-color,box-shadow] duration-200 hover:-translate-y-0.5 hover:border-brand-500/40 hover:shadow-e2 focus-visible:-translate-y-0.5 focus-visible:border-brand-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/30"
      >
