...
export default function ThreatHunting(): JSX.Element {
  ...
  const fetchHunts = useCallback(async () => {
    ...
    try {
      ...
      if (huntsRes.ok) {
        const d = (await huntsRes.json()) as { hunts: Hunt[] };
        setHunts(d.hunts);
      }
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 text-slate-900 dark:text-slate-100">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-display font-bold flex items-center gap-3">
            <Target className="text-brand-600" /> Threat Hunting
          </h1>
          <p className="text-slate-600 dark:text-slate-400 mt-1">
            Hypothesis-driven hunting framework — create, track, and document hunts
          </p>
        </div>
        <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {templates.map((t) => (
              <button
                ...
                className="text-left p-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-brand-300 dark:hover:border-brand-700 transition-colors"
              >
                <h3 className="font-semibold text-xs mb-1">{t.name}</h3>
                <p className="text-micro text-slate-500 line-clamp-2">{t.hypothesis_template}</p>
                <div className="flex flex-wrap gap-1 mt-2">
                  {t.mitre_techniques.slice(0, 3).map((m) => (
                    <span key={m} className="font-mono text-micro text-slate-400">
                      {m}
                    </span>
                  ))}
                </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-slate-500">No hunts found. Create your first hypothesis-driven hunt.</p>
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-2 text-xs text-slate-400">
                      ...
                      {h.mitre_techniques.length > 0 && (
                        <span className="font-mono">{h.mitre_techniques.slice(0, 3).join(', ')}</span>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-6 max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                <span className="block text-xs font-medium text-slate-500 mb-1">Hypothesis</span>
                <textarea
                  value={newHunt.hypothesis}
                  onChange={(e) => setNewHunt({ ...newHunt, hypothesis: e.target.value })}
                  rows={3}
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm"
                  placeholder="An adversary has gained access and is..."
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewHunt({ ...newHunt, priority: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm"
                  >
                ...
                <div>
                  <span className="block text-xs font-medium text-slate-500 mb-1">Kill Chain Phase</span>
                  <input
                    value={newHunt.kill_chain_phase}
                    onChange={(e) => setNewHunt({ ...newHunt, kill_chain_phase: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm"
                    placeholder="lateral-movement"
                  />
                </div>
              </div>
              <div>
                <span className="block text-xs font-medium text-slate-500 mb-1">Query</span>
                <textarea
                  value={newHunt.query}
                  onChange={(e) => setNewHunt({ ...newHunt, query: e.target.value })}
                  rows={3}
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-mono text-xs"
                  placeholder="SecurityEvent | where EventID == 4624..."
                />
              </div>
              <div className="flex gap-2">
                <button
                  onClick={createHunt}
                  disabled={!newHunt.title || !newHunt.hypothesis}
