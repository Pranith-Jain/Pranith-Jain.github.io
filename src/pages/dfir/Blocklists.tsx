...
const FORMATS = [
  { key: 'pfsense', label: 'pfSense', desc: 'Newline-separated IPs for pfSense URL alias (Alias) import', icon: Shield, ext: 'txt' },
  { key: 'iptables', label: 'iptables', desc: 'Shell script with INPUT/FORWARD DROP rules', icon: Terminal, ext: 'sh' },
  { key: 'suricata', label: 'Suricata', desc: 'Suricata drop rules with auto-incrementing SID', icon: Activity, ext: 'rules' },
] as const;
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-8 py-12 text-slate-900 dark:text-slate-100">
      ...
      <div className="animate-fade-in-up">
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-8 max-w-3xl">
          Daily-generated blocklists from cross-source IOC consensus. IPs appearing in 2+ independent feeds.
          Download for pfSense, iptables, or Suricata. Updated every 24 hours.
        </p>
      </div>
        ...
        <div className="flex items-center gap-3 text-sm font-mono">
          ...
          {loading ? (
            ...
            <span>
              ...
              {meta.source === 'kv' && (
                <span className="ml-2 text-[10px] uppercase tracking-wider text-emerald-600 dark:text-emerald-400">cached</span>
              )}
            </span>
          ) : null}
        </div>
        ...
        {FORMATS.map((fmt) => (
          ...
          >
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => downloadFormat(fmt.key, fmt.ext)}
      ...
      <div className="mt-8 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
        ...
        <div className="space-y-2 text-xs font-mono text-slate-600 dark:text-slate-400">
          <p><span className="text-brand-600 dark:text-brand-400">pfSense:</span> Add the URL as an Alias of type URL (URL Alias) in Firewall → Aliases.</p>
          <p><span className="text-brand-600 dark:text-brand-400">iptables:</span> Run <code className="px-1 py-0.5 rounded bg-slate-100 dark:bg-slate-800">chmod +x blocklist-iptables.sh &amp;&amp; sudo ./blocklist-iptables.sh</code></p>
          <p><span className="text-brand-600 dark:text-brand-400">Suricata:</span> Place the rules file in <code className="px-1 py-0.5 rounded bg-slate-100 dark:bg-slate-800">/etc/suricata/rules/</code> and add it to your suricata.yaml.</p>
          <p className="text-[10px] text-slate-400 mt-2">API endpoints: <code className="px-1 py-0.5 rounded bg-slate-100 dark:bg-slate-800">/api/v1/blocklists/pfsense</code>, <code className="px-1 py-0.5 rounded bg-slate-100 dark:bg-slate-800">/api/v1/blocklists/iptables</code>, <code className="px-1 py-0.5 rounded bg-slate-100 dark:bg-slate-800">/api/v1/blocklists/suricata</code></p>
        </div>
      </div>
    </div>
  );
