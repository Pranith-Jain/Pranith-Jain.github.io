    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 text-slate-900 dark:text-slate-100">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-display font-bold flex items-center gap-3">
            <Zap className="text-brand-600" /> Playbook Engine
          </h1>
          <p className="text-slate-600 dark:text-slate-400 mt-1">
            Automated incident response workflows — execute, track, and audit
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={seedTemplates}
            className="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-sm hover:bg-slate-50 dark:hover:bg-slate-800"
          >
            Seed Templates
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-slate-500">No playbooks. Click "Seed Templates" to load defaults.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {playbooks.map((pb) => (
            <button
              ...
              className="text-left p-4 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-brand-300 dark:hover:border-brand-700 transition-colors"
            >
              <div className="flex items-start justify-between mb-2">
                <div>
                  <h3 className="font-semibold text-sm">{pb.name}</h3>
                  <span className="text-micro font-mono uppercase text-slate-400">{pb.category}</span>
                </div>
                <span
                  className={`px-2 py-0.5 rounded text-micro font-semibold ${pb.status === 'active' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-800'}`}
                >
                  {pb.status}
                </span>
              </div>
              ...
              <div className="flex items-center gap-3 text-xs text-slate-400">
                ...
                {pb.mitre_techniques.length > 0 && (
                  <span className="font-mono">{pb.mitre_techniques.slice(0, 2).join(', ')}</span>
                )}
              </div>
              <div className="flex flex-wrap gap-1 mt-2">
                {pb.tags.slice(0, 4).map((t) => (
                  <span
                    key={t}
                    className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-micro text-slate-500"
                  >
                    {t}
                  </span>
                ))}
          ...
          <div className="w-full max-w-lg rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-6 max-h-[80vh] overflow-y-auto">
              ...
              <div className="space-y-1">
                {(selected.steps as Array<{ name: string; type: string }>).map((s, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="font-medium">{s.name}</span> <span className="text-slate-400">({s.type})</span>
                  </div>
                ))}
              </div>
            </div>
            {(
              selected.inputs as Array<{
                name: string;
                label: string;
                required: boolean;
                type: string;
                ...
                {(
                  ...
                  <div key={inp.name}>
                    <span className="block text-xs font-medium text-slate-500 mb-1">
                      {inp.label} {inp.required && <span className="text-rose-500">*</span>}
                    </span>
            ...
            <div className="flex gap-2">
              <button
                onClick={execute}
                disabled={executing}
                className="flex-1 px-4 py-2 rounded-lg bg-brand-600 hover:bg-brand-700 text-white text-sm font-medium disabled:opacity-50 inline-flex items-center justify-center gap-2"
              >
