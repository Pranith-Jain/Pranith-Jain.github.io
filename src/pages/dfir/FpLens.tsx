    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-8 py-12 text-slate-900 dark:text-slate-100">
      ...
      <div className="animate-fade-in-up mb-10">
        <h1 className="text-3xl sm:text-4xl font-display font-bold mb-2 flex items-center gap-3">
          <ScanSearch size={28} className="text-brand-600 dark:text-brand-400" /> FPLENS
        </h1>
        <p className="text-slate-600 dark:text-slate-400 max-w-2xl leading-relaxed">
          False Positive Likelihood Analyzer. Paste a detection rule (Sigma, KQL, SPL, XQL, or just an alert name) plus
          optional sample hits and environment context. The model returns a structured verdict — FP risk level, plausible
          FP patterns with signals, TP indicators, suggested exclusions, and tuning guidance.
        </p>
        ...
        <div className="space-y-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {EXAMPLE_RULES.map((ex) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-md text-xs font-medium border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-brand-400 hover:text-brand-700 dark:hover:text-brand-300 transition-colors"
                >
                  Load: {ex.label}
                </button>
              ))}
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 shadow-e1 p-5">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 p-3 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500/40"
            />
          </div>
          ...
          <div className="flex gap-3">
            ...
            {(result || error) && (
              <button
                ...
                className="px-3 py-3 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                title="Clear"
              >
                <RotateCcw size={16} />
              </button>
              ...
              <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 shadow-e1 p-5">
                ...
                <div className="space-y-3">
                  {result.fp_patterns.map((p, i) => (
                    <div
                      key={i}
                      className="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 p-3"
                    >
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">{p.signals}</p>
                    </div>
                  ))}
                </div>
              </div>
