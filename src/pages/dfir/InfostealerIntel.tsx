...
export default function InfostealerIntel(): JSX.Element {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-8 py-12 text-slate-900 dark:text-slate-100">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                tab === t.id
                  ...
                  : 'border border-slate-200 dark:border-slate-700 text-muted hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-100'
              }`}
            >
        ...
        <div className="flex gap-2">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-brand-500 dark:focus:border-brand-400 font-mono"
            />
          </div>
          <button
          ...
          {(searchResult || overviewResult || infectionResult) && (
            <button
              ...
              className="px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-muted hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
      ...
      {!loading && searchResult && (
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-4 text-sm text-muted">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700 font-mono text-xs">
              v3
            </span>
            <span>
              Infections:{' '}
          ...
          {!searchResult.found && (
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 shadow-e1 p-8 text-center text-muted">
          ...
          {searchResult.results.map((entry, i) => (
            ...
            >
              <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center gap-2.5 text-sm">
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {entry.credentials.map((cred, j) => (
                  <div key={j} className="px-4 py-2 flex flex-wrap items-center gap-2.5 text-xs">
                    <span
                      className={`inline-block px-2 py-0.5 rounded-full border text-xs ${TYPE_COLORS[cred.type] ?? 'bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700'}`}
                    >
                      {cred.type}
                    </span>
                    <span
      ...
      {/* Footer */}
      <div className="mt-8 pt-4 border-t border-slate-200 dark:border-slate-800 text-xs text-slate-400 flex items-center gap-2">
