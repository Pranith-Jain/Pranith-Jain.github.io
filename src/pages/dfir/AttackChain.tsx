...
const TACTIC_COLORS: Record<string, string> = {
  TA0043: 'bg-slate-500', TA0042: 'bg-slate-600', TA0001: 'bg-rose-500', TA0002: 'bg-orange-500',
  TA0003: 'bg-amber-500', TA0004: 'bg-yellow-600', TA0005: 'bg-emerald-500', TA0006: 'bg-teal-500',
  TA0007: 'bg-blue-500', TA0008: 'bg-indigo-500', TA0009: 'bg-purple-500', TA0011: 'bg-pink-500',
  TA0010: 'bg-rose-600', TA0040: 'bg-red-700',
};
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-8 py-12 text-slate-900 dark:text-slate-100">
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-5 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
          <div>
            <label className="text-xs text-slate-500 dark:text-slate-400 mb-1 block">Known Malware (comma-separated)</label>
            <input type="text" value={malware} onChange={(e) => setMalware(e.target.value)} placeholder="e.g., Cobalt Strike, Emotet" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-brand-500 dark:focus:border-brand-400" />
          </div>
          <div>
            <label className="text-xs text-slate-500 dark:text-slate-400 mb-1 block">Known Actors (comma-separated)</label>
            <input type="text" value={actors} onChange={(e) => setActors(e.target.value)} placeholder="e.g., APT28, Lazarus" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-brand-500 dark:focus:border-brand-400" />
          </div>
        </div>
        <button onClick={handleReconstruct} disabled={loading || !input.trim()} className="mt-4 px-5 py-2.5 bg-brand-600 hover:bg-brand-500 disabled:bg-slate-300 dark:disabled:bg-slate-700 disabled:cursor-not-allowed rounded-lg text-sm font-semibold text-white transition-colors flex items-center gap-2">
        ...
        <div className="space-y-5 animate-fade-in-up">
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-2xl font-display font-bold text-brand-600 dark:text-brand-400">{result.kill_chain_progress}%</span>
            </div>
            <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-3 mb-3">
              <div className="bg-gradient-to-r from-brand-600 to-brand-400 h-3 rounded-full transition-all" style={{ width: `${result.kill_chain_progress}%` }} />
            </div>
            <div className="flex flex-wrap gap-1.5 text-xs font-mono">
              <span className="px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700 text-slate-500">{coveredTactics.length} tactics</span>
              <span className="px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700 text-slate-500">{totalTechniques} techniques</span>
              <span className="px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700 text-slate-500">{result.indicators.length} IOCs</span>
            </div>
          </div>
            ...
            <div className="space-y-1.5">
              {result.tactics.map((t, i) => (
                <div key={t.tactic.id} className="flex items-center gap-3">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white ${t.coverage > 0 ? TACTIC_COLORS[t.tactic.id] ?? 'bg-slate-500' : 'bg-slate-200 dark:bg-slate-800 text-slate-400'}`}>
                    {i + 1}
                  </div>
                  <div className={`flex-1 p-3 rounded-lg border ${t.coverage > 0 ? 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/40' : 'border-slate-100 dark:border-slate-800/50 bg-slate-50 dark:bg-slate-950'}`}>
                    <div className="flex items-center justify-between">
                      <span className={`text-sm font-medium ${t.coverage > 0 ? 'text-slate-900 dark:text-white' : 'text-slate-400'}`}>{t.tactic.name}</span>
                      {t.coverage > 0 && (
                        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300">{t.techniques.length} techniques</span>
                      )}
                    </div>
                    {t.coverage > 0 && t.techniques.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {t.techniques.map((tech) => (
                          <span key={tech.id} className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-700 text-slate-500">{tech.id}: {tech.name}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  {i < result.tactics.length - 1 && <ChevronRight size={14} className="text-slate-300 dark:text-slate-600 shrink-0" />}
                </div>
              ))}
            </div>
          </div>
          ...
          {result.predicted_next && (
            <div className="rounded-xl border border-amber-300/50 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-950/20 p-5">
              ...
              <div className="text-sm font-medium text-amber-800 dark:text-amber-200 mb-1">{result.predicted_next.tactic.name}</div>
              ...
              {result.predicted_next.techniques.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {result.predicted_next.techniques.map((t, i) => (
                    <span key={i} className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-amber-300/50 dark:border-amber-800/50 text-amber-700 dark:text-amber-300">{t}</span>
                  ))}
                </div>
              )}
            </div>
          ...
          {result.gaps.length > 0 && (
            <div className="rounded-xl border border-orange-300/50 dark:border-orange-800/50 bg-orange-50 dark:bg-orange-950/20 p-5">
              <h3 className="font-display font-bold text-sm mb-2 text-orange-700 dark:text-orange-300">Intelligence Gaps</h3>
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-5">
              ...
              <div className="space-y-2">
                {result.recommendations.map((rec, i) => (
                  <div key={i} className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-3">
                    <div>
                    ...
                    <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${PRIORITY_BADGE[rec.priority]}`}>{rec.priority}</span>
                  </div>
                ))}
              </div>
            </div>
