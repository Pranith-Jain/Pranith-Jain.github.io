...
const toSeverity = (v: string): Severity => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 text-slate-900 dark:text-slate-100">
      ...
      <h1 className="text-3xl font-display font-bold flex items-center gap-3 mb-2">
        <Target className="text-brand-600" /> Intelligence Requirements
      </h1>
      <p className="text-slate-600 dark:text-slate-400 mb-8">
        Priority Intelligence Requirements (PIRs) with collection coverage and gap analysis
      </p>
      ...
      {gaps.length > 0 && (
        <div className="mb-8 p-4 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/20">
          <h2 className="font-semibold text-sm flex items-center gap-2 mb-3">
            <AlertTriangle size={14} className="text-amber-600" /> Coverage Gaps ({gaps.length})
          </h2>
          <div className="space-y-2">
            {gaps.map((g, i) => (
              <div key={i} className="text-xs">
                <span className="font-medium">{g.requirement_title}</span>: {g.description} —{' '}
                <span className="text-amber-700 dark:text-amber-300">{g.recommended_action}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-slate-500">No intelligence requirements defined yet.</p>
        </div>
      ) : (
          ...
          {pirs.map((p) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={`px-2 py-0.5 rounded border text-micro font-semibold ${SEVERITY_TONE[toSeverity(p.priority)]}`}
                >
                  {p.priority}
                </span>
                <span
                  className={`px-2 py-0.5 rounded text-micro ${p.status === 'active' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30' : 'bg-slate-100 text-slate-500'}`}
                >
                  {p.status}
                </span>
              </div>
