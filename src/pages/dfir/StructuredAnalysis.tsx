...
export default function StructuredAnalysis(): JSX.Element {
  ...
  const createAnalysis = async () => {
    try {
      const res = await fetch('/api/v1/sat', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ ...newAnalysis, created_by: 'analyst' }),
      });
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 text-slate-900 dark:text-slate-100">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-display font-bold flex items-center gap-3">
            <Brain className="text-brand-600" /> Structured Analytic Techniques
          </h1>
          <p className="text-slate-600 dark:text-slate-400 mt-1">
            Standard intelligence analysis methodologies — ACH, Key Assumptions, Red Team, and more
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 mb-8">
        {SAT_TYPES.map((t) => (
          <button
            key={t.id}
            onClick={() => {
              setNewAnalysis({ ...newAnalysis, type: t.id });
              setShowCreate(true);
            }}
          ...
          >
            ...
            <p className="text-micro text-slate-500">{t.desc}</p>
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-slate-500">No analyses yet. Create your first structured analysis.</p>
        </div>
      ) : (
          ...
          {analyses.map((a) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <span
                  className={`px-2 py-0.5 rounded text-micro ${a.status === 'completed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30' : 'bg-slate-100 text-slate-500 dark:bg-slate-800'}`}
                >
                  {a.status}
                </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-6">
            ...
            <div className="space-y-4">
              <div>
                <span className="block text-xs font-medium text-slate-500 mb-1">Technique</span>
                <select
              ...
              <div>
                <span className="block text-xs font-medium text-slate-500 mb-1">Key Question</span>
                <textarea
                  value={newAnalysis.question}
                  onChange={(e) => setNewAnalysis({ ...newAnalysis, question: e.target.value })}
                  rows={2}
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm"
                />
              </div>
