...
export default function CaseManager(): JSX.Element {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 text-slate-900 dark:text-slate-100">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-display font-bold flex items-center gap-3">
            <Shield className="text-brand-600" /> Case Manager
          </h1>
          <p className="text-slate-600 dark:text-slate-400 mt-1">
            Incident response case tracking with evidence chain-of-custody
          </p>
        </div>
        <button
      ...
      {error && (
        <div className="mb-4 p-3 rounded-lg bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-300 text-sm">
          {error}
        </div>
      )}
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        ...
        >
          ...
          {['open', 'triaging', 'investigating', 'containing', 'eradicating', 'recovering', 'closed'].map((s) => (
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-slate-500">No cases found. Create your first incident case.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {cases.map((c) => (
            <Link
              key={c.id}
              to={`/dfir/cases/${c.id}`}
              className="block p-4 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-brand-300 dark:hover:border-brand-700 transition-colors"
            >
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-micro font-semibold uppercase ${STATUS_COLORS[c.status]}`}
                    >
                      {c.status}
                    </span>
                    <span
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-slate-400">
                    <span className="flex items-center gap-1">
                      <Clock size={12} /> {new Date(c.updated_at).toLocaleDateString()}
                    </span>
                    {c.assigned_to && <span>Assigned: {c.assigned_to}</span>}
                    {c.mitre_techniques.length > 0 && (
                      <span className="font-mono">{c.mitre_techniques.slice(0, 3).join(', ')}</span>
                    )}
                  </div>
                </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ransomware incident on SRV-01"
                />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewCase({ ...newCase, severity: e.target.value as CaseSeverity })}
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm"
                  >
                    {['low', 'medium', 'high', 'critical'].map((s) => (
                      <option key={s} value={s}>
