...
const STATUS_STYLES: Record<CheckStatus, { label: string; cls: string; tone: 'good' | 'warn' | 'bad' | 'muted' }> = {
  unset: {
    label: '— unset',
    cls: 'border-slate-300 dark:border-slate-700 text-slate-500 dark:text-slate-400',
    tone: 'muted',
  },
  covered: {
    label: '✓ covered',
  ...
  na: {
    label: 'n/a',
    cls: 'border-slate-300 dark:border-slate-700 text-slate-400 dark:text-slate-500',
    tone: 'muted',
  },
};
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-8 py-12 text-slate-900 dark:text-slate-100">
      ...
      <div className="animate-fade-in-up">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 font-mono mb-8">
          ...
          <Link to="/threatintel/external-resources" className="text-brand-600 dark:text-brand-400 hover:underline">
          ...
          <ul className="space-y-2">
            {openGaps.slice(0, 12).map(({ item, category }) => {
              ...
              return (
                <li
                  key={item.id}
                  className="rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-3"
                >
