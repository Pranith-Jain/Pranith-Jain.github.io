    ...
    >
      <div className="mb-6 grid grid-cols-2 gap-2 sm:grid-cols-4">
        {CATEGORIES.map((cat) => (
          <div
            key={cat.id}
            className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-e1 px-3 py-2"
          >
        ...
        {CATEGORIES.map((cat) => (
          <section key={cat.id} aria-label={cat.label}>
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="rounded-full border border-slate-300 dark:border-slate-700 px-2 py-0.5 text-micro font-mono text-slate-500 dark:text-slate-400">
                {cat.tools.length}
              </span>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {cat.tools.map((tool) => {
                ...
                return (
                  <div
                    key={tool.name}
                    className="surface-card overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-e1 transition-all"
                  >
                    <div className="p-4">
                    ...
                    {open && (
                      <div
                        id={`detail-${key}`}
                        className="border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 p-4 space-y-3 animate-fade-in-up"
                      >
                        <div>
                          <span className="text-micro font-mono uppercase tracking-wider text-slate-500 dark:text-slate-400">
                            Install
                          </span>
                          <pre className="mt-1 overflow-x-auto rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-3 py-2 text-xs font-mono text-slate-800 dark:text-slate-200">
                            {tool.detail.install}
                          </pre>
                        </div>
                        <div>
