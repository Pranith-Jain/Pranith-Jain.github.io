...
const TIER_LABELS: Record<number, { label: string; color: string }> = {
  1: { label: 'Advisory', color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' },
  2: { label: 'Exploit', color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' },
  3: { label: 'Research', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' },
  4: { label: 'Vendor', color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' },
  5: { label: 'Community', color: 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300' },
};
...
export default function CyberNewsFeed(): JSX.Element {
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      const data = await api.get<NewsResult>(`/api/v1/cyber-news?${params}`, { timeoutMs: 25000 });
      setResult(data);
    } catch (err) {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-white dark:bg-[rgb(var(--surface-200))] border border-slate-300 dark:border-[rgb(var(--border-400))] rounded text-sm"
          />
        </div>
        <button
          onClick={fetchNews}
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {Object.entries(TIER_LABELS).map(([tier, { label, color }]) => (
          <button
            ...
            className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${
              activeTier === Number(tier) ? 'bg-brand-600 text-white' : `${color} hover:opacity-80`
            }`}
          >
            {label}
          </button>
      ...
      {result && (
        <div className="text-xs text-slate-500 dark:text-slate-400">
          {filtered?.length ?? 0} articles — updated {relativeAgo(result.last_updated)}
        </div>
      )}
        ...
        {filtered?.map((article, i) => {
          ...
          return (
            <a
              ...
              className="block p-3 bg-white dark:bg-[rgb(var(--surface-200))] border border-slate-200 dark:border-[rgb(var(--border-400))] rounded hover:border-brand-300 dark:hover:border-brand-600 transition-colors group"
            >
              <div className="flex items-start gap-3">
                {article.image_url && (
                  <img
                    ...
                    alt=""
                    className="w-16 h-12 object-cover rounded flex-shrink-0 hidden sm:block"
                    loading="lazy"
                  />
                )}
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`px-1.5 py-0.5 text-[10px] font-medium rounded ${tier.color}`}>{tier.label}</span>
