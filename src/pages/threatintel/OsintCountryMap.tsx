...
const CATEGORY_ORDER = [
  'osint-portal', 'bookmarks', 'regional-guide', 'github',
  'community', 'research', 'company-registry', 'government',
  'guide', 'general',
];
...
const NUMERIC_TO_ALPHA2: Record<string, string> = {
  ...
  '418': 'LA', '422': 'LB', '426': 'LS', '430': 'LR', '434': 'LY',
  '440': 'LT', '442': 'LU', '450': 'MG', '454': 'MW', '458': 'MY',
  '462': 'MV', '466': 'ML', '478': 'MR', '480': 'MU', '484': 'MX',
  ...
  '634': 'QA', '642': 'RO', '643': 'RU', '646': 'RW', '662': 'LC',
  '670': 'VC', '678': 'ST', '682': 'SA', '686': 'SN', '694': 'SL',
...
const MAX_RESOURCE_COUNT = Math.max(
  1,
  ...OSINT_COUNTRIES.map(c => c.resources.length)
);
...
export default function OsintCountryMap(): JSX.Element {
  ...
  const filteredCountries = useMemo(() => {
    ...
    return OSINT_COUNTRIES.filter(c =>
      c.name.toLowerCase().includes(q) ||
      c.resources.some(r => r.name.toLowerCase().includes(q))
    ).slice(0, 30);
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={selectedCountry ? `Search within ${selectedCountry.name}...` : 'Search countries or resources...'}
            className="w-full pl-9 pr-4 py-2 rounded-lg border border-zinc-700 bg-zinc-800/50 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-zinc-500"
          />
          {query && (
            <button
              onClick={() => setQuery('')}
        ...
        <div className="flex gap-4 flex-col lg:flex-row">
          ...
          <div className="lg:w-3/5 w-full">
            <div className="rounded-lg border border-zinc-700 bg-zinc-800/30 overflow-hidden">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-zinc-300">
                    {selectedCountry
                      ...
                      : `${OSINT_COUNTRIES.length} countries`}
                  </span>
                </div>
              ...
              <Suspense fallback={<div className="h-[400px] flex items-center justify-center text-zinc-500 text-sm">Loading map...</div>}>
                <OsintMapChart
                  topoUrl={TOPO_URL}
            ...
            {!selectedCountry && query && (
              <div className="mt-2 rounded-lg border border-zinc-700 bg-zinc-800/30 max-h-60 overflow-y-auto">
                {filteredCountries.length === 0 ? (
                  <div className="p-4 text-sm text-zinc-500 text-center">No countries match "{query}"</div>
                ) : (
                  filteredCountries.map(c => (
                    <button
                      key={c.alpha2}
                      onClick={() => { handleSelect(c.alpha2); setQuery(''); }}
                      className="w-full text-left px-4 py-2 text-sm text-zinc-300 hover:bg-zinc-700/50 flex items-center gap-2"
                    >
                      <span className="text-zinc-500">{c.name}</span>
                      <span className="text-xs text-zinc-600 ml-auto">{c.resources.length} resources</span>
                    </button>
                  ))
                )}
              </div>
          ...
          <div className="lg:w-2/5 w-full">
            {selectedCountry ? (
              <div className="rounded-lg border border-zinc-700 bg-zinc-800/30 flex flex-col h-[500px]">
                {/* Header */}
                <div className="p-3 border-b border-zinc-700/50 flex items-center justify-between shrink-0">
                  <div>
                    <h3 className="text-sm font-semibold text-zinc-100">{selectedCountry.name}</h3>
                    <p className="text-xs text-zinc-500">{selectedResources.length} of {selectedCountry.resources.length} resources</p>
                  </div>
                  <button
                ...
                {ALL_CATEGORIES.length > 0 && (
                  <div className="px-3 py-2 border-b border-zinc-700/50 flex flex-wrap gap-1.5 shrink-0">
              ...
              <div className="rounded-lg border border-zinc-700 bg-zinc-800/30 h-[500px] flex items-center justify-center">
                <div className="text-center p-6">
                  <Globe className="w-10 h-10 text-zinc-600 mx-auto mb-3" />
                  <p className="text-sm text-zinc-400 mb-1">Click a country on the map</p>
...
function ResourceRow({ resource }: { resource: OsintCountryResource }): JSX.Element {
  ...
  return (
    <div className="px-3 py-2 hover:bg-zinc-700/20 transition-colors group">
      <div className="flex items-start gap-2">
        ...
        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-zinc-700/50 text-zinc-400 shrink-0">
      ...
      <button
        ...
        className="text-[11px] text-zinc-600 hover:text-zinc-400 mt-0.5 truncate max-w-full block"
        title="Copy URL"
      >
