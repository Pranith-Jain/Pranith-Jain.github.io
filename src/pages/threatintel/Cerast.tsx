    ...
    <>
      ...
      >
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-1.5">
            {EXAMPLES.map((ex) => (
              <button
                ...
                className="px-2.5 py-1 rounded-md border border-[rgb(var(--border-400))] bg-[rgb(var(--surface-200))] text-[11px] font-mono text-muted hover:border-brand-500/40 hover:text-brand-600 dark:hover:text-brand-400 hover:bg-[rgb(var(--surface-300))] transition-all"
              >
                {ex}
              </button>
            ))}
          ...
          {/* Error */}
          {error && (
            <div className="flex items-center gap-2 p-3 rounded-lg border border-rose-300/50 dark:border-rose-800/40 bg-rose-50/60 dark:bg-rose-950/30 text-rose-700 dark:text-rose-400 text-sm">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                {data.results.length > 0 && (
                  <div className="flex items-center gap-1.5 text-[11px] text-muted">
                    ...
                    {data.results.filter(r => r.impact === 'HIGH').length} high · {data.results.filter(r => r.impact === 'MEDIUM').length} medium
                  </div>
                )}
              </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {data.results.map((r, i) => (
                        <tr key={`${r.domain}-${r.path}-${i}`} className="border-b border-[rgb(var(--border-400))] last:border-b-0 hover:bg-[rgb(var(--surface-300))]/40 transition-colors group">
                          <td className="px-4 py-2.5 font-mono text-xs">
                            <span className={r.multihost ? 'opacity-50' : 'text-foreground group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors'}>{r.domain}</span>
                          ...
                          <td className="px-4 py-2.5">
                            <span className="px-2 py-0.5 rounded-full border border-[rgb(var(--border-400))] bg-[rgb(var(--surface-300))]/50 text-[11px] text-muted">{r.category}</span>
                          </td>
                          <td className="px-4 py-2.5">
                ...
                <div className="text-[11px] text-muted space-y-1 pl-1">
                  {data.diagnostics.map((d, i) => (
                    <div key={i} className="font-mono">
                      <span className="text-foreground/60">{d.provider}</span>: {d.status} <span className="text-foreground/40">({d.ms}ms)</span>{d.error ? <span className="text-rose-500/70"> — {d.error}</span> : ''}
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="mt-6 p-5 rounded-xl border border-[rgb(var(--border-400))] bg-[rgb(var(--surface-200))] text-xs text-muted space-y-3">
            ...
            <p>
              <a href="https://search.cerast-intelligence.com" target="_blank" rel="noopener noreferrer" className="text-brand-600 dark:text-brand-400 hover:underline inline-flex items-center gap-1 font-medium">
