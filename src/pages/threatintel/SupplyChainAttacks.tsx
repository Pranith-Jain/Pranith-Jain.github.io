...
export default function SupplyChainAttacks(): JSX.Element {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch('/api/v1/supply-chain-attacks', { signal: ctrl.signal })
      .then((r) => {
  ...
  const ecosystems = useMemo(
    () => Object.entries(data?.facets.ecosystems ?? {}).sort((a, b) => b[1] - a[1]),
    [data]
  );
    ...
    >
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <h3 className="font-semibold text-sm text-slate-900 dark:text-slate-100 leading-snug">
                  {titleHref ? (
                    <a href={titleHref} target="_blank" rel="noopener noreferrer" className="hover:text-brand-600 dark:hover:text-brand-400">
                ...
                {inc.severity && (
                  <span className={`shrink-0 text-micro font-mono uppercase tracking-wider px-1.5 py-0.5 rounded border ${SEVERITY_TONE[inc.severity.toLowerCase()] ?? SEVERITY_TONE.low}`}>
                    {inc.severity}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                {inc.status && (
                  <span className={`text-micro font-mono uppercase tracking-wider px-1.5 py-0.5 rounded border ${STATUS_TONE[inc.status.toLowerCase()] ?? STATUS_TONE.disputed}`}>
                    {inc.status}
                  </span>
                )}
                {inc.ecosystems.map((e) => (
                  <span key={e} className="text-micro font-mono px-1.5 py-0.5 rounded border border-sky-500/40 text-sky-600 dark:text-sky-400">
                    {e}
                  </span>
                ))}
                {inc.attack_vectors.map((v) => (
                  <span key={v} className="text-micro font-mono px-1.5 py-0.5 rounded border border-violet-500/40 text-violet-600 dark:text-violet-400">
                    {v}
                  </span>
                ))}
                {inc.disclosed_date && <span className="text-micro font-mono text-slate-400 ml-auto">{inc.disclosed_date}</span>}
              </div>

              {inc.summary && <p className="text-xs text-slate-600 dark:text-slate-400 mt-2 leading-relaxed">{inc.summary}</p>}
      ...
      {data && (
        <p className="mt-6 text-micro font-mono text-slate-400 text-center">
          Data:{' '}
          <a href={data.source_url} target="_blank" rel="noopener noreferrer" className="hover:text-brand-600 dark:hover:text-brand-400">
            {data.source}
          </a>{' '}
