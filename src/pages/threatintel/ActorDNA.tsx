...
export default function ActorDNA(): JSX.Element {
  ...
  const matchTTPs = async () => {
    ...
    setMatches([]);
    try {
      const ttps = ttpsInput.split(',').map(s => s.trim()).filter(Boolean);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-8 py-12 text-slate-900 dark:text-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <BackLink to="/threatintel" className="inline-flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400 hover:text-brand-600 dark:text-brand-400 mb-8 font-mono">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold">Threat Actor Behavioral DNA</h1>
        ...
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              !matchMode ? 'bg-brand-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'
            }`}
          >
          ...
          <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800 p-6 mb-6">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white dark:bg-slate-900/40 border border-slate-600 rounded-lg px-4 py-2.5 text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:border-purple-500"
              />
              <button
                onClick={matchTTPs}
                disabled={loading}
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800 p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-bold text-brand-600 dark:text-brand-400">{selectedActor.confidence}%</div>
                ...
                <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800">
                  ...
                  >
                    ...
                    {expandedSections.has('ttp') ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                  </button>
                  {expandedSections.has('ttp') && (
                    <div className="px-4 pb-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                      <DNACard title="Initial Access" items={selectedActor.ttp_signature.preferred_initial_access} color="red" />
                      <DNACard title="Execution" items={selectedActor.ttp_signature.preferred_execution} color="orange" />
                      <DNACard title="Persistence" items={selectedActor.ttp_signature.persistence_patterns} color="yellow" />
                ...
                <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800">
                  ...
                  >
                    ...
                    {expandedSections.has('victimology') ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                  </button>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedActor.victimology.preferred_sectors.map((s, i) => (
                            <span key={i} className="text-sm px-3 py-1 bg-red-500/10 text-red-400 rounded-full">{s}</span>
                          ))}
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedActor.victimology.preferred_regions.map((r, i) => (
                            <span key={i} className="text-sm px-3 py-1 bg-blue-500/10 text-blue-400 rounded-full">{r}</span>
                          ))}
                        </div>
                      </div>
                ...
                <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800">
                  ...
                  >
                    ...
                    {expandedSections.has('tempo') ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                  </button>
                  {expandedSections.has('tempo') && (
                    <div className="px-4 pb-4 grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <div className="text-sm">{selectedActor.operational_tempo.active_hours_utc[0]}:00 - {selectedActor.operational_tempo.active_hours_utc[1]}:00</div>
                      </div>
                      <div>
                        <div className="text-xs text-slate-500">Campaign Duration</div>
                        <div className="text-sm">{selectedActor.operational_tempo.campaign_duration_avg_days} days avg</div>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="text-sm capitalize">{selectedActor.operational_tempo.seasonal_pattern.replace(/_/g, ' ')}</div>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800">
                  ...
                  >
                    ...
                    {expandedSections.has('infra') ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                  </button>
                  {expandedSections.has('infra') && (
                    <div className="px-4 pb-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                      <DNACard title="Hosting" items={selectedActor.infrastructure_dna.hosting_preferences} color="cyan" />
                      <DNACard title="Domain Patterns" items={selectedActor.infrastructure_dna.domain_patterns} color="pink" />
                      <DNACard title="SSL Patterns" items={selectedActor.infrastructure_dna.ssl_patterns} color="indigo" />
