...
interface Props { actors: Actor[]; feed: FeedItem[]; }
...
export function LandscapeView({ actors, feed }: Props) {
  ...
  const filteredActors = useMemo(() => {
    ...
    if (search) {
      ...
      items = items.filter(a =>
        a.name.toLowerCase().includes(q) || a.apt?.toLowerCase().includes(q) ||
        a.description.toLowerCase().includes(q) || a.agency.toLowerCase().includes(q) ||
  ...
  const stats = useMemo(() => {
    ...
    filteredActors.forEach(a => { motivations[a.motivation] = (motivations[a.motivation] || 0) + 1; });

    const sectors: Record<string, number> = {};
    filteredActors.forEach(a => a.sectors.forEach(s => { sectors[s] = (sectors[s] || 0) + 1; }));
    const sortedSectors = Object.entries(sectors).sort((a, b) => b[1] - a[1]).slice(0, 10);

    const nationCounts: Record<string, number> = {};
    filteredActors.forEach(a => { nationCounts[a.country] = (nationCounts[a.country] || 0) + 1; });

    const avgActive = filteredActors.length > 0 ? Math.round(filteredActors.reduce((s, a) => s + (2025 - a.active_since), 0) / filteredActors.length) : 0;
  ...
  const recentFeed = useMemo(() => {
    ...
    if (search) {
      ...
      items = items.filter(f =>
        f.title.toLowerCase().includes(q) || f.source.toLowerCase().includes(q) ||
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-6 space-y-6 animate-fade-in-up">
        <header>
          ...
          <p className="text-slate-600 dark:text-slate-400 text-[13px]">
            Executive brief on {filteredActors.length} tracked actor{filteredActors.length !== 1 ? 's' : ''} · {stats.avgActive}yr avg operational lifespan · {stats.totalCampaigns} known campaigns
          </p>
        </header>
        ...
        <div className="surface-card p-4">
          <div className="flex items-center gap-3 flex-wrap">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                ...
                className="w-full h-9 pl-9 pr-3 rounded-lg text-[13px] text-slate-600 dark:text-slate-400 placeholder:text-slate-500 dark:placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-500/30 transition-all"
            ...
            <button onClick={() => setShowFilters(!showFilters)}
              ...
              style={{ borderColor: showFilters ? 'rgba(91,141,239,0.4)' : 'var(--edge)', color: showFilters ? 'var(--accent-blue)' : 'var(--text-secondary)', background: showFilters ? 'rgba(91,141,239,0.08)' : 'var(--ink-700)' }}>
          ...
          {showFilters && (
            <div className="mt-3 pt-3 border-t flex items-center gap-3 flex-wrap" style={{ borderColor: 'var(--edge)' }}>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono uppercase tracking-wider text-slate-500 dark:text-slate-400">Sector</span>
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono uppercase tracking-wider text-slate-500 dark:text-slate-400">Motivation</span>
                <select value={motivationFilter ?? ''} onChange={e => setMotivationFilter(e.target.value || null)}
                  className="h-6 px-2 rounded text-[11px] text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-[rgb(var(--border-400))] bg-white dark:bg-[rgb(var(--surface-200))] outline-none capitalize">
                  <option value="">All motivations</option>
                  {allMotivations.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono uppercase tracking-wider text-slate-500 dark:text-slate-400">Sort</span>
                {(['threat', 'name', 'date'] as const).map(s => (
                  <button key={s} onClick={() => setSortBy(s)}
                    ...
                    style={sortBy === s
                      ? { background: 'rgba(91,141,239,0.12)', color: 'var(--accent-blue)', borderColor: 'rgba(91,141,239,0.4)' }
              ...
              {(sectorFilter || motivationFilter || search) && (
                <button onClick={() => { setSectorFilter(null); setMotivationFilter(null); setSearch(''); }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="surface-card p-5">
            <div className="text-eyebrow font-mono text-slate-500 dark:text-slate-400 mb-4">Sector Targeting Heatmap</div>
                ...
                return (
                  <div key={sector}>
                    <div className="flex justify-between text-[12px] mb-1">
                      ...
                      <span className="text-slate-500 dark:text-slate-400 font-mono">{count} actor{count !== 1 ? 's' : ''}</span>
                    </div>
          ...
          <div className="surface-card p-5">
            ...
            <div className="space-y-4">
              {Object.entries(stats.motivations).sort((a, b) => b[1] - a[1]).map(([mot, count]) => {
                ...
                return (
                  <div key={mot}>
                    <div className="flex items-center gap-3 mb-1.5">
                      <span className="text-[12px] text-slate-600 dark:text-slate-400 capitalize font-medium w-28 shrink-0">{mot}</span>
                      <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: 'var(--ink-600)' }}>
                        <div className="h-full rounded-full bg-accent/60 transition-all duration-500" style={{ width: `${pct}%` }} />
                      </div>
                      <span className="text-[11px] font-mono text-slate-500 dark:text-slate-400 w-8 text-right">{count}</span>
                    </div>
                    <div className="flex flex-wrap gap-1 ml-31">
                      {motivationActors.map(a => (
                        <span key={a.id} className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ background: `${NATION_PALETTE[a.country]?.color}15`, color: NATION_PALETTE[a.country]?.color }}>
                          {a.name}
                        </span>
                      ))}
                    </div>
          ...
          <div className="surface-card p-5">
            ...
            <div className="space-y-2">
              {Object.entries(stats.nationCounts).sort((a, b) => b[1] - a[1]).map(([code, count]) => {
                const n = NATION_PALETTE[code];
                const nationActors = filteredActors.filter(a => a.country === code);
                return (
                  <div key={code} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-[rgb(var(--surface-300))] transition-colors">
                    <span
                      className="h-7 w-9 rounded flex items-center justify-center text-[10px] font-mono font-bold shrink-0"
                      style={{ background: `${n?.color ?? '#555'}20`, color: n?.color ?? '#888' }}
                    >
                      {code}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="text-[12px] text-slate-600 dark:text-slate-400 font-medium">{n?.name ?? code}</div>
                      <div className="text-[10px] font-mono text-slate-500 dark:text-slate-400 truncate">
                        {nationActors.map(a => a.name).join(', ')}
                      </div>
                    </div>
                    <span className="text-[12px] font-mono font-semibold text-slate-900 dark:text-slate-100 shrink-0">{count}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div className="surface-card p-5">
            ...
            <div className="space-y-2">
              {recentFeed.length === 0 && <p className="text-slate-500 dark:text-slate-400 text-[12px]">No recent items match current actors.</p>}
              {recentFeed.map(f => (
                <a
                  key={f.id}
                  href={f.url}
                ...
                >
                  <div className="text-[12.5px] text-slate-600 dark:text-slate-400 leading-snug group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors">{f.title}</div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400">{f.source}</span>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400">·</span>
                    <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400">{f.published}</span>
                  </div>
                </a>
              ))}
            </div>
...
function KPI({ label, value, icon: Icon }: {
  label: string; value: number; icon: typeof ShieldAlert;
}) {
  return (
    <div className="surface-card p-4 flex items-center gap-3">
      ...
      <div>
        <div className="text-xl sm:text-2xl font-bold font-mono text-slate-900 dark:text-slate-100">{value.toLocaleString()}</div>
        <div className="text-[10px] font-mono uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">{label}</div>
      </div>
    </div>
  );
}
