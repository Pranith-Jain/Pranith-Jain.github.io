...
function StatPills({ stats }: { stats: BriefingStats | undefined }) {
  if (!stats) return <span className="text-slate-500">—</span>;
  const empty = isEmpty(stats);
  return (
    <span className={`font-mono text-xs ${empty ? 'text-rose-300' : 'text-slate-300'}`}>
      {num(stats.findings)} findings · {num(stats.iocs)} IOCs · {num(stats.cves)} CVEs · {num(stats.kevs)} KEVs
      {empty && <span className="ml-2 px-1.5 py-0.5 rounded bg-rose-900/40 border border-rose-800">EMPTY</span>}
    </span>
  );
}
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="rounded border border-slate-800 p-4">
        <h2 className="text-lg font-semibold text-slate-100 mb-1">Build now</h2>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          {(['daily', 'weekly', 'landscape'] as BuildType[]).map((t) => (
            <button
              ...
              className="px-4 py-1.5 border border-slate-700 rounded text-sm text-slate-200 hover:bg-slate-800 disabled:opacity-50 capitalize"
            >
          ...
          <button
            ...
            className="px-4 py-1.5 border border-emerald-700 text-emerald-200 rounded text-sm hover:bg-emerald-900/30 disabled:opacity-50"
          >
            {building === 'daily-live' ? 'Building today…' : 'Build today (live)'}
          </button>
        </div>
        {buildError && (
          <div className="px-3 py-2 border border-rose-700 bg-rose-900/20 rounded text-sm text-rose-200">
            {buildError}
          </div>
        )}
        {buildResult && (
          <div className="px-3 py-2 border border-slate-700 bg-slate-900 rounded text-sm">
            <span className={buildResult.ok ? 'text-emerald-300 font-semibold' : 'text-amber-300 font-semibold'}>
              {buildResult.ok ? 'Built' : 'Done'}
            </span>{' '}
            <code className="font-mono text-slate-300">{buildResult.slug}</code>
            {buildResult.reason && <span className="text-slate-500"> — {buildResult.reason}</span>}
            <div className="mt-1">
      ...
      {/* Backfill */}
      <div className="rounded border border-slate-800 p-4">
        <h2 className="text-lg font-semibold text-slate-100 mb-1">Backfill</h2>
        <p className="text-sm text-slate-400 mb-4">
        ...
        <div className="flex flex-wrap items-end gap-3 mb-3">
          <label className="block">
            <span className="block text-xs uppercase tracking-wider text-slate-500 mb-1">Daily (0–21)</span>
            <input
              type="number"
              min={0}
              max={21}
              ...
              className="w-24 px-2 py-1 bg-slate-900 border border-slate-700 rounded text-sm text-slate-100 disabled:opacity-50"
            />
          </label>
          <label className="block">
            <span className="block text-xs uppercase tracking-wider text-slate-500 mb-1">Weekly (0–4)</span>
            <input
              type="number"
              min={0}
              max={4}
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded text-sm disabled:opacity-50 ${
              force
                ? 'border border-rose-700 text-rose-200 hover:bg-rose-900/30'
        ...
        {backfillResult && (
          <div className="px-3 py-2 border border-slate-700 bg-slate-900 rounded text-sm text-slate-300 space-y-1">
            <div>
              <span className="text-emerald-300">written:</span> daily {backfillResult.daily.length}, weekly{' '}
              {backfillResult.weekly.length} · <span className="text-slate-500">skipped:</span> daily{' '}
              {backfillResult.daily_skipped.length}, weekly {backfillResult.weekly_skipped.length}
              {backfillResult.failures.length > 0 && (
                <span className="text-rose-300"> · failures: {backfillResult.failures.length}</span>
              )}
            </div>
            {backfillResult.daily.length > 0 && (
              <div className="font-mono text-xs text-slate-400 break-all">{backfillResult.daily.join(', ')}</div>
            )}
          </div>
        )}
      </div>
      ...
      {/* Sweep */}
      <div className="rounded border border-slate-800 p-4">
        <h2 className="text-lg font-semibold text-slate-100 mb-1">Sweep old briefings</h2>
        <p className="text-sm text-slate-400 mb-4">
        ...
        <div className="flex flex-wrap items-end gap-3 mb-3">
          <label className="block">
            <span className="block text-xs uppercase tracking-wider text-slate-500 mb-1">Max age (days)</span>
            <input
              type="number"
              min={1}
              max={3650}
              ...
              disabled={sweeping}
              className="w-28 px-2 py-1 bg-slate-900 border border-slate-700 rounded text-sm text-slate-100 disabled:opacity-50"
            />
          </label>
          <button
            onClick={runSweep}
            disabled={sweeping}
            className="px-4 py-1.5 border border-rose-700 text-rose-200 rounded text-sm hover:bg-rose-900/30 disabled:opacity-50"
          >
        ...
        {sweepResult && (
          <div className="px-3 py-2 border border-slate-700 bg-slate-900 rounded text-sm text-slate-300">
            <span className="text-emerald-300 font-semibold">Swept</span> — {sweepResult.deleted.length} deleted,{' '}
      ...
      <div className="rounded border border-slate-800 p-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold text-slate-100">Recent briefings</h2>
          <div className="flex items-center gap-2">
            <button
              onClick={pruneEmpty}
              disabled={pruning || !anyEmpty}
              title={anyEmpty ? 'Delete all empty daily/weekly briefings' : 'No empty briefings to prune'}
              className="px-3 py-1 border border-rose-700 text-rose-200 rounded text-sm hover:bg-rose-900/30 disabled:opacity-40"
            >
            ...
            <button
              ...
              className="px-3 py-1 border border-slate-700 rounded text-sm text-slate-300 hover:bg-slate-800 disabled:opacity-50"
            >
        ...
        {deleteMsg && (
          <div className="mb-3 px-3 py-2 border border-slate-700 bg-slate-900 rounded text-sm text-slate-300">
            {deleteMsg}
          </div>
        )}
        {listError && (
          <div className="mb-3 px-3 py-2 border border-rose-700 bg-rose-900/20 rounded text-sm text-rose-200">
            {listError}
          </div>
        )}
        <div className="border border-slate-800 rounded overflow-hidden">
          <table className="w-full text-sm">
            <thead className="border-b border-slate-800 text-slate-500 uppercase text-xs tracking-wider">
              <tr>
            ...
            <tbody>
              {items.map((it) => (
                <tr
                  key={it.slug}
                  className={`border-t border-slate-800/50 ${isEmpty(it.metadata.stats) ? 'bg-rose-950/20' : ''}`}
                >
                  <td className="px-3 py-1.5 font-mono text-slate-300">{it.slug}</td>
                  <td className="px-3 py-1.5 text-slate-400 capitalize">{it.metadata.type ?? '—'}</td>
                  ...
                  <td className="px-3 py-1.5 text-right">
                    <button
                      ...
                      className="px-2 py-0.5 border border-rose-800 text-rose-300 rounded text-xs hover:bg-rose-900/30 disabled:opacity-40"
                    >
