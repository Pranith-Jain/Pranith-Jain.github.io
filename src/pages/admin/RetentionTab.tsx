...
export default function RetentionTab() {
  ...
  return (
    <div className="rounded border border-slate-800 p-4 max-w-3xl">
      <h2 className="text-lg font-semibold text-slate-100 mb-1">Data retention sweep</h2>
      ...
      <div className="flex flex-wrap items-end gap-3 mb-4">
        <label className="block">
          <span className="block text-xs uppercase tracking-wider text-slate-500 mb-1">Retention (days)</span>
          <input
            type="number"
            min={1}
            max={3650}
            ...
            disabled={busy !== null}
            className="w-32 px-2 py-1 bg-slate-900 border border-slate-700 rounded text-sm text-slate-100 disabled:opacity-50"
          />
        </label>
        <button
          onClick={() => run(true)}
          disabled={busy !== null}
          className="px-4 py-1.5 border border-slate-700 rounded text-sm text-slate-200 hover:bg-slate-800 disabled:opacity-50"
        >
      ...
      {error && (
        <div className="mb-4 px-3 py-2 border border-rose-700 bg-rose-900/20 rounded text-sm text-rose-200">
          {error}
        </div>
      )}
      ...
      {result && (
        <div className="mt-4">
          <div className="text-sm text-slate-300 mb-3">
            {wasDry ? (
              <>
                <span className="text-amber-300 font-semibold">Dry run</span> — {totalDeleted} row(s) would be deleted
                (no DELETEs issued).
              </>
            ) : (
              <>
                <span className="text-emerald-300 font-semibold">Sweep complete</span> — {totalDeleted} row(s) deleted.
              </>
            )}
            {cutoff && (
              <>
                {' '}
                Cutoff: <code className="font-mono text-slate-400">{cutoff.toISOString()}</code>.
              </>
            )}
            {ranAt && (
              <>
                {' '}
                Ran at: <code className="font-mono text-slate-400">{ranAt.toLocaleString()}</code>.
              </>
            )}
          </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {result.tables_swept.map((row) => (
                    <tr key={row.table} className="border-t border-slate-800/50">
                      <td className="px-3 py-1.5 font-mono text-slate-300">{row.table}</td>
                      <td className="px-3 py-1.5 text-right text-slate-100">{row.deleted}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {result.tables_swept.length === 0 && (
            <p className="text-sm text-slate-500">No tables had rows past the cutoff — nothing to do.</p>
          )}
        </div>
      )}
