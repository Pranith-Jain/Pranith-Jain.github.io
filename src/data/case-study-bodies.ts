/**
 * Markdown bodies for the long-form case studies, keyed by slug.
 *
 * Split out of case-studies.ts (2026-07-29 perf audit): the bodies are the
 * bulk of that module (~38KB), and only the /projects/:slug read page needs
 * them - a lazy route. The metadata module, by contrast, is imported eagerly
 * by the home page's Projects section, so keeping the bodies here keeps them
 * out of the entry chunk. The read page is the only importer.
 */

export const CASE_STUDY_BODIES: Record<string, string> = {
  'phishing-program-at-scale': `Most analyst write-ups about "how to triage phishing" stop at the screenshot of headers and the smug "of course it's phishing" conclusion. They skip the part that actually matters at volume: which decisions stay with the human, which get automated away, and what changes when the queue is two-hundred-and-fifty incidents deep instead of two.
...
Reach me at [hello@pranithjain.qzz.io](mailto:hello@pranithjain.qzz.io) if that resonates.`,
  'dmarc-enforcement-1300-domains': `Every DMARC rollout write-up has the same arc: a confident plan, a graph going up and to the right, and the unsaid bit where someone's CFO's auto-forward chain breaks for a week. This is the one with the unsaid bit included.
...
If you've solved that piece for your org and feel like comparing notes, reach me at [hello@pranithjain.qzz.io](mailto:hello@pranithjain.qzz.io).`,
  'email-infra-automation-platform': `Spinning up a new mail-sending domain by hand has six steps. Buy the domain. Wire DNS. Provision Workspace users. Warm up the inboxes. Hook the inboxes into the sending tool. Watch the auth alignment land. Done by a person, that's an afternoon of clicking through six different consoles and a printed-out checklist. Done across 1,300 domains and 2,700 inboxes, that's not a job; that's a job that never finishes.
...
If you're building a similar layer or untangling a similar mess, reach me at [hello@pranithjain.qzz.io](mailto:hello@pranithjain.qzz.io). Happy to compare notes on the parts your vendors lied about.`,
  'dfir-toolkit-design': `A 60-tool toolkit is suspect by default. Most of the time it means someone confused breadth for depth, and clicking around for ten minutes turns up the same JSON-formatter five times with a different name. I wanted to avoid that.
...
If you want to chat about tool design, or you've built a similar surface and want to compare what earned a slot versus what got demoted, reach me at [hello@pranithjain.qzz.io](mailto:hello@pranithjain.qzz.io).`,
  'threat-intel-platform-build': `The promise of an AI-written threat-intel blog is high. Discover topics from live feeds, generate analysis, publish without a human touching it. The risk is also high. One hallucinated CVE, one fabricated IOC, one invented attribution, and the blog becomes a liability instead of an asset.
...
/** Look up a case study body by slug. Empty string for unknown slugs. */
export function getCaseStudyBody(slug: string): string {
  return CASE_STUDY_BODIES[slug] ?? '';
}
