...
export const THREATINTEL_FAQ: { question: string; answer: string }[] = [
  {
    question: 'What is the Threat Intelligence Platform?',
    answer:
      'The Threat Intelligence Platform is a free, live, self-updating CTI surface that aggregates 30-plus intel sources — ransomware leak sites, MITRE ATT&CK, CISA KEV, NVD, deepdarkCTI criminal forums, Telegram channels, and Reddit subreddits — into one searchable view. It correlates IOCs across sources with consensus scoring, tracks threat actor timelines, monitors dark-web chatter, and exports enriched bundles as STIX 2.1. Everything updates hourly with no signup required.',
  },
  {
    question: 'How does the platform pull from ransomware leak sites?',
    answer:
      'The platform pulls leak-site claims from six independent sources: Ransomlook, ransomfeed.it, ransomwatch, ransomware.live, MyThreatIntel, and Andrea Fortuna. Each is fetched in parallel, normalised into a common victim schema, deduplicated by leak-site claim, and ranked by recency. The result is one feed of recent victims with per-victim screenshots, sector heuristics, and a re-leak timeline so analysts can spot repeat targets rather than scrolling raw rows.',
  },
  {
  ...
  {
    question: 'Is the Threat Intel platform free?',
    answer:
      'Yes. The platform is free, with no signup, no API key, and no rate-limit login. It runs on Cloudflare Workers using a KV cache and a D1 database, and is sustained by the same sponsorship page that covers the rest of the site. Server costs are absorbed by the author, not by ads or data resale. The platform is not affiliated with any commercial threat-intel vendor; it is a side project by a working SOC analyst.',
  },
];
