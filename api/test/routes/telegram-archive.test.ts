...
function stubCache(byKey: Record<string, unknown>) {
  vi.stubGlobal('caches', {
    default: {
      match: async (key: string) =>
        key in byKey ? new Response(JSON.stringify(byKey[key]), { status: 200 }) : undefined,
    },
  });
}
...
describe('runTelegramArchive', () => {
  it('is a no-op when not configured', async () => {
    ...
    const res = await runTelegramArchive({ CASE_STUDIES: kv() } as never);
  ...
  it('posts new items, dedups on the next run, persists state', async () => {
    ...
    const env = { TELEGRAM_BOT_TOKEN: 't', TELEGRAM_CHANNEL_ID: '@c', CASE_STUDIES: kv() } as never;
  ...
  it('bails without updating state when Telegram rate-limits', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response('{"ok":false}', { status: 429 }))
    );
    const k = kv();
    const env = { TELEGRAM_BOT_TOKEN: 't', TELEGRAM_CHANNEL_ID: '@c', CASE_STUDIES: k } as never;
    const r = await runTelegramArchive(env);
