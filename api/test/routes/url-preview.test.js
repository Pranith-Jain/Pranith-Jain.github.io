...
describe('GET /api/v1/url-preview', () => {
    ...
    it('returns 400 on unsupported protocol (data:)', async () => {
        const r = await SELF.fetch('https://x/api/v1/url-preview?url=' + encodeURIComponent('data:text/html,<h1>test</h1>'));
        expect(r.status).toBe(400);
        const body = (await r.json());
        expect(body.error).toMatch(/unsupported protocol/i);
    });
    it('returns 403 when DNS resolves to private IP', async () => {
        ...
        const r = await SELF.fetch('https://x/api/v1/url-preview?url=' + encodeURIComponent('http://internal-host.example.com/'));
    ...
    it('returns 502 when upstream throws', async () => {
        ...
        const r = await SELF.fetch('https://x/api/v1/url-preview?url=' + encodeURIComponent('https://unreachable.example.com/'));
