...
import { cryptoWatchAddHandler, cryptoWatchListHandler, cryptoWatchRemoveHandler, cryptoAlertsHandler, } from '../../src/routes/crypto-monitor';
function app() {
    const a = new Hono();
...
const env = () => ({ ...testEnv, ADMIN_TOKEN: 'sekret' });
...
describe('crypto-monitor (admin, mini-app)', () => {
    ...
    it('400 on a bad chain', async () => {
        const r = await app().request('/api/v1/crypto-monitor/watch', {
            method: 'POST',
            headers: bearer,
    ...
    it('add -> list -> alerts -> delete round-trip', async () => {
        const add = await app().request('/api/v1/crypto-monitor/watch', {
            method: 'POST',
            headers: bearer,
        ...
        const alerts = await app().request('/api/v1/crypto-monitor/alerts?address=0xWATCHED1&chain=evm', { headers: bearer }, env());
        expect(alerts.status).toBe(200);
        const del = await app().request('/api/v1/crypto-monitor/watch/0xWATCHED1/evm', { method: 'DELETE', headers: bearer }, env());
