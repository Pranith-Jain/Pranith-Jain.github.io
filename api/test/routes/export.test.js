...
import { exportStixHandler, exportMispHandler, exportSigmaHandler, exportYaraHandler, exportSnortHandler, exportSuricataHandler, exportCsvHandler, exportPfSenseHandler, } from '../../src/routes/export';
function setup() {
    const app = new Hono();
...
function post(app, path, body) {
    return app.request(path, { method: 'POST', body, headers: { 'content-type': 'application/json' } }, {});
}
const ioc = {
    value: '1.2.3.4',
    type: 'ip',
...
describe('export hub routes', () => {
    ...
    it('YARA: object body with name → 200 .yar download', async () => {
        const app = setup();
        const r = await post(app, '/api/v1/export/yara', JSON.stringify({ name: 'rule1', hash_iocs: ['abc'], string_iocs: ['evil'] }));
        expect(r.status).toBe(200);
