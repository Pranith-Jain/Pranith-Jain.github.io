...
const APT28_BODY = 'APT28 (Fancy Bear) was observed conducting spear-phishing using CVE-2024-21762 to deliver the CremShell malware. ' +
...
describe('GET /api/v1/intel-bundle', () => {
    ...
    it('computes + persists on miss when title+body supplied; second call serves from D1', { timeout: 20_000 }, async () => {
        const url = new URL('https://example.com/api/v1/intel-bundle');
        url.searchParams.set('source', 'briefings');
...
describe('POST /api/v1/intel-bundle/build', () => {
    ...
    it('builds from a flat IoC list (iocs mode) — emits indicators even without prose context', { timeout: 20_000 }, async () => {
...
describe('GET /api/v1/intel-bundle/:id/export.stix.json', () => {
    ...
    it('returns 404 when the bundle ID is well-formed but not persisted', async () => {
        const res = await SELF.fetch('https://example.com/api/v1/intel-bundle/bundle--00000000-0000-0000-0000-000000000000/export.stix.json');
        expect(res.status).toBe(404);
    });
    it('serves the persisted STIX bundle with the correct media type + attachment header', { timeout: 20_000 }, async () => {
        ...
        await new Promise((r) => setTimeout(r, 50));
        const res = await SELF.fetch(`https://example.com/api/v1/intel-bundle/${encodeURIComponent(id)}/export.stix.json`);
        expect(res.status).toBe(200);
