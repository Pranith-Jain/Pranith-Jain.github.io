...
describe('POST /api/v1/sample/scan — input validation', () => {
    it('rejects a missing hash', async () => {
        const r = await app().request('/api/v1/sample/scan', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({}) }, TEST_ENV);
        expect(r.status).toBe(400);
    });
    it('rejects a malformed hash (not hex / wrong length)', async () => {
        const r = await app().request('/api/v1/sample/scan', {
            method: 'POST',
    ...
    it('rejects invalid JSON', async () => {
        const r = await app().request('/api/v1/sample/scan', { method: 'POST', headers: { 'content-type': 'application/json' }, body: 'this is not json' }, TEST_ENV);
        expect(r.status).toBe(400);
    });
