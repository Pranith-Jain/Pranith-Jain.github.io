...
describe('GET /api/v1/feeds/proxy', () => {
    ...
    it('rejects host not in allow-list', async () => {
        const r = await SELF.fetch('https://x/api/v1/feeds/proxy?url=' + encodeURIComponent('https://evil.example.com/feed.xml'));
    ...
    it('proxies allowed host', async () => {
        ...
        const r = await SELF.fetch('https://x/api/v1/feeds/proxy?url=' + encodeURIComponent('https://www.cisa.gov/feed.xml'));
