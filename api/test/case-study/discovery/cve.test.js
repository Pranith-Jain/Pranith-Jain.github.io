...
describe('discoverCves', () => {
    it('returns candidates from KEV with kev=true severity', async () => {
        const fetchMock = vi.fn(async (url) => {
            if (url.includes('cisa.gov'))
                return new Response(JSON.stringify(fakeKev));
            return new Response(JSON.stringify({ vulnerabilities: [] }));
        });
        const now = new Date('2026-05-14T06:00:00Z');
        const cands = await discoverCves({ fetch: fetchMock, now, getDedup: async () => null });
    ...
    it('penalizes novelty if previously seen', async () => {
        ...
        const now = new Date('2026-05-14T06:00:00Z');
        const dedup = async (_key) => ({ lastSeenAt: now.toISOString() });
        const cands = await discoverCves({ fetch: fetchMock, now, getDedup: dedup });
