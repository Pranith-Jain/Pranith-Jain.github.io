...
import { discoverReleaks } from '../../src/case-study/discovery/releak';
function row(o) {
    return {
        raw_names: [o.key],
        claims: [],
...
describe('discoverReleaks', () => {
    ...
    it('uses `releak-<victim-key>` for the dedup key', async () => {
        const rows = [row({ key: 'acme', group_count: 2, latest: '2026-05-15T00:00:00Z' })];
        const out = await discoverReleaks({ fetchReleaks: async () => rows, now, getDedup: async () => null });
    ...
    it('subtype:releak is set on every candidate so the prompt can lean into the narrative', async () => {
        const rows = [row({ key: 'x', group_count: 2, latest: '2026-05-19T00:00:00Z', raw_names: ['x inc'] })];
        const out = await discoverReleaks({ fetchReleaks: async () => rows, now, getDedup: async () => null });
        expect(out[0].evidence.subtype).toBe('releak');
    });
});
