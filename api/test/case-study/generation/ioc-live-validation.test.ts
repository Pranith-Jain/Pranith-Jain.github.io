...
import type { PostIOC } from '../../../src/case-study/types';

/**
 * Mocked fetch matrix. Each call inspects the URL + body to decide which
 * provider it is and what to return. Each test installs the matrix it
 * wants and asserts against the validator's output.
 */

type Reply = { status: number; body?: unknown };

function installFetch(matrix: (url: string, init?: RequestInit) => Reply | Promise<Reply>) {
...
const goodIp: PostIOC = { type: 'ipv4', value: '91.215.155.42' };
const badIp: PostIOC = { type: 'ipv4', value: '198.51.100.99' };
const goodHash: PostIOC = {
  type: 'sha256',
  value: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
};
const goodDomain: PostIOC = { type: 'domain', value: 'malware.example.org' };
...
describe('validateIocsLive', () => {
  ...
  it('drops an IPv4 when VT and AbuseIPDB both say "absent"', async () => {
    installFetch((url) => {
      if (url.includes('virustotal.com')) return { status: 404 };
      if (url.includes('abuseipdb.com')) return { status: 200, body: { data: { totalReports: 0 } } };
      return { status: 500 };
    });
  ...
  it('keeps an IPv4 when at least one provider returns 200', async () => {
    installFetch((url) => {
      if (url.includes('virustotal.com')) return { status: 200, body: { data: {} } };
      if (url.includes('abuseipdb.com')) return { status: 200, body: { data: { totalReports: 0 } } };
      return { status: 500 };
    });
  ...
  it('drops a sha256 when MalwareBazaar returns hash_not_found and VT 404s', async () => {
    installFetch((url) => {
      if (url.includes('virustotal.com')) return { status: 404 };
      if (url.includes('mb-api.abuse.ch')) return { status: 200, body: { query_status: 'hash_not_found' } };
      return { status: 500 };
    });
  ...
  it('keeps a sha256 when MalwareBazaar says ok', async () => {
    installFetch((url) => {
      if (url.includes('virustotal.com')) return { status: 404 };
      if (url.includes('mb-api.abuse.ch')) return { status: 200, body: { query_status: 'ok', data: [{}] } };
      return { status: 500 };
    });
    const r = await validateIocsLive([goodHash], { VT_API_KEY: 'vt', ABUSECH_AUTH_KEY: 'ach' });
    expect(r.iocs).toHaveLength(1);
  ...
  it('keeps an IOC when no configured provider supports its type (e.g. email)', async () => {
    ...
    const email: PostIOC = { type: 'email', value: 'attacker@example.test' };
  ...
  it('caps validations at MAX_VALIDATIONS — IOCs beyond the cap pass through unchanged', async () => {
    ...
    const many: PostIOC[] = Array.from({ length: 30 }, (_, i) => ({
      type: 'ipv4',
  ...
  it('handles a mix of keep/drop in one pass', async () => {
    installFetch((url) => {
      if (url.includes('virustotal.com')) {
        ...
        if (url.includes('91.215.155.42')) return { status: 200, body: { data: {} } };
        return { status: 404 };
      }
      if (url.includes('abuseipdb.com')) {
        if (url.includes('91.215.155.42')) return { status: 200, body: { data: { totalReports: 5 } } };
