...
const allOk = async (urls) => new Map(urls.map((u) => [u, 'ok']));
...
describe('generatePost', () => {
  it('produces a complete Post for an approved candidate', async () => {
    ...
    runCompletion.mockResolvedValue({ text: goodMd, modelUsed: 'mock' });
  ...
  it('throws if post-processing rejects the output', async () => {
    const { runCompletion } = await import('../../../src/case-study/generation/ai-client');
    runCompletion.mockResolvedValue({ text: 'Garbage with no sections.', modelUsed: 'mock' });
    const { generatePost } = await import('../../../src/case-study/generation/index');
    const ai = { run: vi.fn() };
  ...
  it('a QA-failing first draft is rescued by the one-shot repair pass', async () => {
    ...
    const { generatePost } = await import('../../../src/case-study/generation/index');
    const ai = { run: vi.fn() };
    const post = await generatePost({ candidate, ai: ai, now: new Date(), verifyRefs: allOk });
  ...
  it('throws "qa failed" when content stays sub-standard after repair', async () => {
    ...
    const { runCompletion } = await import('../../../src/case-study/generation/ai-client');
    runCompletion.mockResolvedValue({ text: thin, modelUsed: 'mock' });
    const { generatePost } = await import('../../../src/case-study/generation/index');
    const ai = { run: vi.fn() };
  ...
  it('prunes a confirmed-broken reference URL from the published post body', async () => {
    ...
    const { runCompletion } = await import('../../../src/case-study/generation/ai-client');
    runCompletion.mockResolvedValue({ text: withBrokenRef, modelUsed: 'mock' });
    const { generatePost } = await import('../../../src/case-study/generation/index');
    const ai = { run: vi.fn() };
    const post = await generatePost({
      candidate,
      ...
      verifyRefs: async (urls) =>
        new Map(urls.map((u) => [u, u.includes('this-slug-does-not-exist') ? 'broken' : 'ok'])),
    });
