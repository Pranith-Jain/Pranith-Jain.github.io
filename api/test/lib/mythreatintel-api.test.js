...
const envWithToken = { MYTHREATINTEL_API_TOKEN: 'test-token' };
const envNoToken = {};
describe('isMtiSource', () => {
    it('accepts every documented source', () => {
        for (const s of MTI_SOURCES)
            expect(isMtiSource(s)).toBe(true);
    });
...
describe('fetchMtiSource', () => {
    ...
    it('normalizes the {status,metadata,data} envelope', async () => {
        ...
        expect(r.items[0].sha256).toBe('a'.repeat(64));
    });
    ...
    it('clamps limit into 1–500 and sends a Bearer token', async () => {
        const spy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ status: 'success', metadata: {}, data: [] }), {
            status: 200,
