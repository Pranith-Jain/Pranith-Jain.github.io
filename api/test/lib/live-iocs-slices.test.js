...
import { sliceKey, writeSlice, readSlice, SLICE_KEY_PREFIX, SLICE_TTL_SECONDS, } from '../../src/lib/live-iocs-slices';
...
async function clearSlice(sourceId) {
...
describe('live-iocs slices (Cache API)', () => {
    ...
    it('returns null when the stored shape is wrong (missing arrays)', async () => {
        await cache.put(sliceKey('demo'), new Response(JSON.stringify({ source_id: 'demo', generated_at: 'x' }), {
