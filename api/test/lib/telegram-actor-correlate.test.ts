...
function fakeCache(payloads: Record<string, unknown>): Cache {
  const map = new Map<string, Response>(Object.entries(payloads).map(([k, v]) => [k, new Response(JSON.stringify(v))]));
  return {
