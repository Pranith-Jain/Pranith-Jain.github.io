...
const fixtureIocs = (n) => {
    ...
    for (let i = 0; i < n; i++) {
        if (i % 5 === 0)
            out.push({ type: 'hash', value: `${i.toString(16).padStart(64, '0')}` });
        else if (i % 5 === 1)
            out.push({ type: 'url', value: `https://example-${i}.test/path` });
        else if (i % 5 === 2)
            out.push({ type: 'domain', value: `bad-${i}.example` });
        else if (i % 5 === 3)
            out.push({ type: 'ipv4', value: `192.0.2.${i % 254}` });
        else
            out.push({ type: 'email', value: `attacker-${i}@example.test` });
    }
    return out;
};
