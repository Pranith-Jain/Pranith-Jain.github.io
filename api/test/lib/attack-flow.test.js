...
import { orderByTactic, buildAttackFlowObjects, TACTIC_ORDER, ATTACK_FLOW_EXT_ID, } from '../../src/lib/attack-flow';
// Deterministic fake id factory: `${type}--${sha-ish of key}` — here just type+key.
const fakeIdFor = async (type, key) => `${type}--${key
    .replace(/[^a-z0-9]/gi, '')
    .slice(0, 8)
    .padEnd(8, '0')}`;
...
describe('buildAttackFlowObjects', () => {
    ...
    it('builds extension-definition + attack-flow + linear attack-actions', async () => {
        ...
        expect(extDef.extension_types).toEqual(['new-sdo']);
        const flow = r.objects.find((o) => o.type === 'attack-flow');
        ...
        const actions = r.objects.filter((o) => o.type === 'attack-action');
        ...
        for (const a of actions)
            expect(a.extensions[ATTACK_FLOW_EXT_ID]).toEqual({ extension_type: 'new-sdo' });
    });
});
