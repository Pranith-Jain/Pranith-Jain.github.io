...
import {
  ...
  type TifceHistoryRow,
} from '../../src/lib/tifce';
import type { LiveIoc, LiveSource } from '../../src/routes/live-iocs';
import type { CorrelatedIoc } from '../../src/routes/ioc-correlation';

const NOW = Date.parse('2026-06-04T12:00:00.000Z');

function liveIoc(value: string, source: string, observed_at?: string): LiveIoc {
  return { value, kind: 'ip', source, observed_at };
}

function feed(id: string, items: LiveIoc[], newest?: string): FeedContribution {
  return {
    feedId: id,
    items,
    source: {
...
describe('freshnessPillar', () => {
  ...
  it('velocity_per_day is the correct reverse mapping of velocity_component (regression: was *50 instead of -50)', () => {
    ...
    const risingHistory: TifceHistoryRow[] = [
    ...
    expect(Math.abs(vpd as number)).toBeLessThan(50);
  });
});
...
describe('buildCrossFeedIndex', () => {
  it('indexes correlated IOCs by value with their source_count and sources list', () => {
    const correlated: CorrelatedIoc[] = [
