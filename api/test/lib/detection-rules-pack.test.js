...
describe('DETECTION_RULES_PACK structure', () => {
    ...
    it('every regex compiles without throwing', () => {
        for (const r of DETECTION_RULES_PACK) {
            if (r.match.valueRegex)
                expect(() => new RegExp(r.match.valueRegex, 'i')).not.toThrow();
            if (r.match.contextRegex)
                expect(() => new RegExp(r.match.contextRegex, 'i')).not.toThrow();
            if (r.match.reporterRegex)
                expect(() => new RegExp(r.match.reporterRegex, 'i')).not.toThrow();
        }
    });
    it('aggregate groupBy uses canonical fields', () => {
        ...
        for (const r of DETECTION_RULES_PACK) {
            if (r.aggregate)
                expect(allowed.has(r.aggregate.groupBy)).toBe(true);
        }
    });
...
describe('DETECTION_RULES_PACK semantics', () => {
    ...
    it('does NOT misfire the low-severity slate rule with a green-looking benign context', () => {
        ...
        const items = [{ value: '198.51.100.5', kind: 'ip', source: 'sans-isc', context: 'patched' }];
        const { detections } = evaluateRules(DETECTION_RULES_PACK, items);
