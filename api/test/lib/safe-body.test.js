...
function makeCtx(rawBody) {
    return {
        req: { text: async () => rawBody },
        json: (body, status = 200, headers = {}) => new Response(JSON.stringify(body), { status, headers: { ...headers, 'content-type': 'application/json' } }),
    };
}
describe('safeJsonBody', () => {
    it('returns the typed value for a valid small payload', async () => {
        ...
        const r = await safeJsonBody(c, { maxBytes: 1024 });
        expect('value' in r).toBe(true);
        if ('value' in r)
            expect(r.value.slug).toBe('x');
    });
    ...
    it('rejects JSON deeper than maxDepth with 400', async () => {
        ...
        const r = await safeJsonBody(c, { maxBytes: 4096, maxDepth: 5 });
        expect('error' in r).toBe(true);
        if ('error' in r) {
            expect(r.error.status).toBe(400);
            const body = (await r.error.json());
    ...
    it('handles arrays in the depth check', async () => {
        ...
        const r = await safeJsonBody(c, { maxBytes: 1024, maxDepth: 3 });
        expect('error' in r).toBe(true);
    });
});
