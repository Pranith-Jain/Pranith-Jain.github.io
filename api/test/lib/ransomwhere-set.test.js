...
describe('buildRansomMap', () => {
    ...
    it('returns an empty map for non-array input', () => {
        ...
        expect(buildRansomMap({ result: [] }).size).toBe(0);
    });
});
describe('checkRansomwhere', () => {
    const map = buildRansomMap(RESULT);
    it('flags an EVM hit case-insensitively (checksummed + lowercased both match)', () => {
        // EVM_ADDR is mixed-case (EIP-55 checksummed); the map key is its lowercase form.
        expect(checkRansomwhere(map, 'evm', EVM_ADDR)).toEqual({ flagged: true, family: 'LockBit' });
        expect(checkRansomwhere(map, 'evm', EVM_ADDR.toLowerCase())).toEqual({ flagged: true, family: 'LockBit' });
    });
    it('flags a BTC hit', () => {
