...
import type { AgentStep, AgentToolCall } from '../../src/lib/agent/types';
import {
  evaluateCtiExit,
  filterCtiToolCalls,
  countOkResults,
...
function referenceExit(stepNum: number, maxSteps: number, ok: number): boolean {
...
function referenceFilter(
  proposed: Array<{ tool: string; args?: Record<string, unknown>; reasoning?: string }>,
  steps: AgentStep[],
...
function stepsWithOk(totalOk: number): AgentStep[] {
  ...
  return totalOk === 0 ? [] : [{ stepNumber: 1, plan: 'p', toolCalls: [], results, status: 'done' as const }];
}

describe('LoopEngine — generic', () => {
  it('evaluateExit returns the first matching condition by order', () => {
    const engine = new LoopEngine<number, never>({
      goal: 'g',
      maxIterations: () => 5,
      exitConditions: [
  ...
  it('applyGuardrails chains filters in order and never mutates input', () => {
    const input = [1, 2, 3, 4];
    const engine = new LoopEngine<null, number>({
      goal: 'g',
      maxIterations: () => 0,
      exitConditions: [],
      guardrails: [
    ...
    for (let maxSteps = 1; maxSteps <= 10; maxSteps++) {
      for (let stepNum = 1; stepNum <= 12; stepNum++) {
        for (let ok = 0; ok <= 8; ok++) {
          const view: CtiLoopView = { stepNum, maxSteps, steps: stepsWithOk(ok) };
...
describe('CTI guardrails — dedup + unknown-tool parity', () => {
  ...
  function priorSteps(executed: Array<{ tool: string; args: Record<string, unknown> }>): AgentStep[] {
    return [
      {
        stepNumber: 1,
        plan: 'p',
        toolCalls: [],
        status: 'done',
        results: executed.map((e) => ({ tool: e.tool, args: e.args, status: 'ok' as const, durationMs: 1 })),
      },
    ];
  }
  ...
  it('drops unknown tools, prior-step dupes, and in-batch dupes — matching reference', () => {
    ...
    const view: CtiLoopView = { stepNum: 2, maxSteps: 6, steps };
    const proposed = [
  ...
  it('passes a well-formed call through untouched (normalization is the planner-parse contract, not the guardrail layer)', () => {
    ...
    const proposed: AgentToolCall[] = [{ tool: 'unified_search', args: {}, reasoning: 'find it' }];
    const got = filterCtiToolCalls(proposed, view, valid);
