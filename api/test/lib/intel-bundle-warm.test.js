...
const env = testEnv;
function fakeBriefing(overrides = {}) {
    return {
                ...
                findings: [
                    {
                        ...
                        description: 'Reports indicate diplo-service.com is hosting payloads. Hash 5d41402abc4b2a76b9719d911017c592 observed.',
                        severity: 'critical',
                        source: 'unit42',
...
async function insertBriefing(db, b) {
    await db
        .prepare(`INSERT INTO briefings
...
beforeAll(async () => {
    ...
    await db.exec(`CREATE TABLE IF NOT EXISTS briefings (` +
    ...
    await db.exec(`CREATE TABLE IF NOT EXISTS intel_bundles (` +
        ` id TEXT PRIMARY KEY,` +
...
describe('warmIntelBundles', () => {
    ...
    it('builds a bundle for a briefing that lacks an intel_bundles row', { timeout: 20_000 }, async () => {
        ...
        expect(r.failed).toEqual([]);
        const row = await env
            .BRIEFINGS_DB.prepare(`SELECT id, source_id, item_ref, ioc_count FROM intel_bundles
    ...
    it('continues past a per-row failure and records it under failed[]', { timeout: 20_000 }, async () => {
        ...
        await env
            .BRIEFINGS_DB.prepare(`INSERT INTO briefings
             (slug, type, title, date, date_range, range_start, range_end, body, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .bind('daily-2026-05-21', 'daily', 'Bad briefing', '2026-05-21', '2026-05-21', '2026-05-21', '2026-05-21', '{not valid json', '2026-05-21T00:30:00Z')
            .run();
    ...
    it('persists LLM-extracted sectors / candidates when extractLlm is wired', { timeout: 30_000 }, async () => {
        ...
        const llmStub = async (_title, _body, _entities, _env, _opts) => ({
        ...
        expect(r.built).toEqual(['daily-2026-05-22']);
        const row = await env
            .BRIEFINGS_DB.prepare(`SELECT view_json FROM intel_bundles WHERE source_id = 'briefings' AND item_ref = 'daily-2026-05-22'`)
            .first();
