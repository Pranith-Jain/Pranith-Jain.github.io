...
describe('extractText dispatcher', () => {
    it('handles plain text in-Worker', async () => {
        const r = await extractText(enc.encode('hello 1.2.3.4'), 'text/plain', 'n.txt', {});
    ...
    it('routes PDF to the bridge → BridgeUnavailable when unset', async () => {
        ...
        await expect(extractText(pdf, 'application/pdf', 'r.pdf', {})).rejects.toBeInstanceOf(BridgeUnavailable);
    });
    it('throws UnsupportedFile for an unknown type', async () => {
        const zip = new Uint8Array([0x50, 0x4b, 0x03, 0x04]);
        await expect(extractText(zip, 'application/zip', 'a.zip', {})).rejects.toBeInstanceOf(UnsupportedFile);
    });
});
describe('sha256Hex', () => {
