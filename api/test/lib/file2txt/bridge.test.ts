...
describe('extractViaBridge', () => {
  it('throws BridgeUnavailable when env is unset', async () => {
    await expect(extractViaBridge(FILE, 'application/pdf', 'r.pdf', {} as never, 'pdf')).rejects.toBeInstanceOf(
      BridgeUnavailable
    );
  });
  ...
  it('posts to the bridge and returns its text', async () => {
    ...
    const env = { FILE2TXT_BRIDGE_URL: 'https://bridge.example', FILE2TXT_BRIDGE_TOKEN: 'tok' } as never;
  ...
  it('throws on non-200 from the bridge', async () => {
    ...
    const env = { FILE2TXT_BRIDGE_URL: 'https://bridge.example' } as never;
