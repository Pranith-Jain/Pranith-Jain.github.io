...
function makeContext(ip, resHeaders = {}) {
  const headers = new Headers();
  for (const [k, v] of Object.entries(resHeaders)) headers.set(k, v);
  const res = new Response(null, { headers });
  return {
    c: {
      req: { header: (name) => (name.toLowerCase() === 'cf-connecting-ip' ? ip : null) },
      res,
      json: (body, status, extraHeaders) => {
        const h = new Headers();
...
describe('burstLimitActorProfile', () => {
  it('allows requests under the limit', async () => {
    ...
    await burstLimitActorProfile(c, async () => {
      nextCalled = true;
    });
  ...
  it('returns 429 with rate-limit headers when over the limit', async () => {
    ...
    const { c } = makeContext(ip);
    const result = await burstLimitActorProfile(c, async () => {});
