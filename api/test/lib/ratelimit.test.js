 ...
 * in lib/ratelimit.ts.
 */
async function seedRateLimit(ip, count) {
 ...
 */
function makeCtx(url, method, ip) {
    return {
        req: {
            url,
            method,
        ...
        json: (body, status, headers) => new Response(JSON.stringify(body), {
            status: status ?? 200,
...
describe('rate limiter — keyed callers get 4x headroom', () => {
    ...
    async function run(url, method, ip, keyed) {
        const ctx = makeCtx(url, method, ip);
        if (keyed)
            ctx.user = { id: 'k1', role: 'readonly' };
        const pending = [];
        ctx.executionCtx = { waitUntil: (p) => pending.push(p) };
        let nextCalled = false;
        const res = await rateLimit(ctx, async () => {
            nextCalled = true;
        });
        await Promise.allSettled(pending);
        return { res: res, nextCalled };
    }
