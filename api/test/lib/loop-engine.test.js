...
import { evaluateCtiExit, filterCtiToolCalls, countOkResults, BANNED_TOOLS, MAX_TOOLS_PER_STEP, } from '../../src/lib/agent/cti-loop';
...
function referenceExit(stepNum, maxSteps, ok) {
    if (stepNum >= maxSteps && ok > 0)
        return true; // planner cond1
    if (stepNum >= maxSteps - 1 && ok >= 3)
        return true; // planner cond2
    if (ok >= 6)
        return true; // planner cond3
    if (stepNum >= maxSteps)
        return true; // DO fallback (ok === 0 at the ceiling)
    return false;
}
...
function referenceFilter(proposed, steps, toolNames) {
    ...
    for (const tc of proposed) {
        if (!tc.tool || !toolNames.has(tc.tool))
            continue;
        ...
        for (let maxSteps = 1; maxSteps <= 10; maxSteps++) {
            for (let stepNum = 1; stepNum <= 12; stepNum++) {
                for (let ok = 0; ok <= 8; ok++) {
                    ...
                    expect(engineExits, `mismatch at stepNum=${stepNum} maxSteps=${maxSteps} ok=${ok}`).toBe(referenceExit(stepNum, maxSteps, ok));
                }
            }
        }
    });
