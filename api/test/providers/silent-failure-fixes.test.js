...
describe('kaspersky — reads top-level Zone (PascalCase) via the request= param', () => {
    it('maps Zone:"Red" to malicious', async () => {
        ...
        const r = await kaspersky({ type: 'ipv4', value: '1.2.3.4' }, { KASPERSKY_API_KEY: 'k' }, AbortSignal.timeout(2000));
    ...
    it('maps Zone:"Green" to clean', async () => {
        vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(new Response(JSON.stringify({ Zone: 'Green' }), { status: 200 }));
        const r = await kaspersky({ type: 'domain', value: 'ok.com' }, { KASPERSKY_API_KEY: 'k' }, AbortSignal.timeout(2000));
...
describe('urlscan — no false "clean"; abstains without a malicious-tag signal', () => {
    it('returns unknown (not clean) when results carry no malicious tags', async () => {
        vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(new Response(JSON.stringify({ results: [{ tags: ['certstream'] }], total: 1 }), { status: 200 }));
    ...
    it('flags suspicious when a result is tagged phishing', async () => {
        vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(new Response(JSON.stringify({ results: [{ tags: ['phishing'] }], total: 1 }), { status: 200 }));
...
describe('censys — org id goes in X-Organization-ID, not concatenated into the Bearer token', () => {
    it('sends Bearer <pat> + X-Organization-ID header', async () => {
        ...
        await censys({ type: 'ipv4', value: '1.2.3.4' }, { CENSYS_PAT: 'PAT', CENSYS_ORG_ID: 'ORG' }, AbortSignal.timeout(2000));
...
describe('yaraify — lookup_hash query + nested data.tasks parsing (was always "clean")', () => {
    const ykEnv = { ABUSECH_AUTH_KEY: 'k' };
    ...
    it('returns clean (not error) on no_result', async () => {
        vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(new Response(JSON.stringify({ query_status: 'no_result' }), { status: 200 }));
