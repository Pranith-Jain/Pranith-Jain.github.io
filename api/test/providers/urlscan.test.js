...
describe('urlscan adapter', () => {
    ...
    it('abstains (unknown, NOT a false clean) when results are empty', async () => {
        vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(new Response(JSON.stringify({ results: [], total: 0 }), { status: 200 }));
        const r = await urlscan({ type: 'domain', value: 'safe.com' }, env, AbortSignal.timeout(2000));
        expect(r.score).toBe(0);
        expect(r.verdict).toBe('unknown');
    });
    it('returns error on 401', async () => {
        vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(new Response('unauthorized', { status: 401, statusText: 'Unauthorized' }));
