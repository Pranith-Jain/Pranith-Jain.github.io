...
describe('secrets adapter', () => {
    ...
    it('returns ok/clean for a URL with no leaked credentials', async () => {
        const r = await secrets({ type: 'url', value: 'https://example.com/safe/path?ok=1' }, env, AbortSignal.timeout(1000));
        expect(r.status).toBe('ok');
        expect(r.verdict).toBe('clean');
        expect(r.score).toBe(0);
        expect(r.tags).toContain('secrets-scan');
        const summary = r.raw_summary;
    ...
    it('returns ok/malicious with redacted findings for a URL with a leaked AWS key', async () => {
        const r = await secrets({ type: 'url', value: 'https://attacker.example/log?k=AKIAIOSFODNN7EXAMPLE' }, env, AbortSignal.timeout(1000));
        expect(r.status).toBe('ok');
    ...
    it('caps the score at 100 even with many findings', async () => {
        const url = 'https://x.example/?a=AKIAIOSFODNN7EXAMPLE&b=ghp_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa&c=sk-abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKL&d=sk_test_fakeKey00000000000000000000000000';
    ...
    it('truncates to at most 10 findings on the wire', async () => {
        ...
        const r = await secrets({ type: 'url', value: `https://x.example/?${params.join('&')}` }, env, AbortSignal.timeout(1000));
        const summary = r.raw_summary;
