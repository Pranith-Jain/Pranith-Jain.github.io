...
import { callerOwnerId, isOperator, ownerCheck } from '../lib/ownership';
...
export async function iocWatchlistCreateHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  // Stored webhooks are POSTed server-side on later sightings — validate at
  // create time (https-only + public host) so a planted URL can't turn the
  // sweep into an exfiltration/spam fan-out. Mirrors watches.ts.
  if (body.alert_channel === 'webhook' || body.webhook_url) {
    if (!body.webhook_url) return badRequest(c, 'webhook_url required for webhook channel');
    let url: URL;
    try {
      url = new URL(body.webhook_url);
    } catch {
      return badRequest(c, 'Invalid webhook URL');
    }
    if (url.protocol !== 'https:') return badRequest(c, 'webhook must be https');
  ...
  const entry = await addWatch(db, {
    ...
    tlp: body.tlp,
    notes: body.notes,
    owner_hash: callerOwnerId(c),
  });

  return c.json(entry, 201);
}

export async function iocWatchlistListHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const owner = isOperator(c) ? undefined : callerOwnerId(c);
  const watches = await listWatches(db, {
    type: type as IocType | undefined,
    limit,
    owner,
  });
...
export async function iocWatchlistGetHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  if (ownerCheck(c, watch.owner_hash ?? null) === 'hidden') return notFound(c, 'not found');
  return c.json(watch);
}

export async function iocWatchlistDeleteHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  const idParam = c.req.param('id');
  const id = idParam ? parseInt(idParam, 10) : NaN;
  if (isNaN(id)) return badRequest(c, 'invalid id');
  ...
  // Integer ids are enumerable — hide foreign rows behind a 404.
  const existing = await getWatch(db, id);
  if (!existing || ownerCheck(c, existing.owner_hash ?? null) === 'hidden') return notFound(c, 'not found');
  await removeWatch(db, id);
  return c.json({ deleted: true });
}

export async function iocWatchlistAlertsHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
