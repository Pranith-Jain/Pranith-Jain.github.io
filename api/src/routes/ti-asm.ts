...
asm.post('/domains', async (c) => {
  ...
  const existing = await db
    .prepare('SELECT id FROM asm_domains WHERE domain = ?')
  ...
  `
    ...
    .bind(id, body.domain.toLowerCase())
    .run();
...
asm.get('/scan', async (c) => {
  ...
  const existingAssets = await db
    .prepare('SELECT value FROM asm_assets WHERE domain_id = ?')
  ...
  for (const asset of assets) {
    if (existingValues.has(asset.value)) {
      asset.status = 'stable';
      await db
        .prepare("UPDATE asm_assets SET last_seen = datetime('now') WHERE domain_id = ? AND value = ?")
        .bind(domainRecord.id, asset.value)
        .run();
    } else {
  ...
  for (const asset of newAssets) {
    ...
    `
      ...
      .bind(asset.id, domainRecord.id, asset.type, asset.value, JSON.stringify(asset.metadata))
      .run();
  }
  ...
  // Update last scan
  await db.prepare("UPDATE asm_domains SET last_scan = datetime('now') WHERE id = ?").bind(domainRecord.id).run();
...
asm.get('/summary', async (c) => {
  ...
  const stats = await Promise.all([
    db
      .prepare('SELECT COUNT(*) as cnt FROM asm_domains')
      .first<{ cnt: number }>()
      .catch(() => ({ cnt: 0 })),
    db
      .prepare('SELECT COUNT(*) as cnt FROM asm_assets')
      .first<{ cnt: number }>()
      .catch(() => ({ cnt: 0 })),
    db
      .prepare("SELECT COUNT(*) as cnt FROM asm_changes WHERE created_at > datetime('now', '-24 hours')")
      .first<{ cnt: number }>()
      .catch(() => ({ cnt: 0 })),
    db
      .prepare(
        "SELECT COUNT(*) as cnt FROM asm_changes WHERE severity = 'high' AND created_at > datetime('now', '-24 hours')"
      )
      .first<{ cnt: number }>()
      .catch(() => ({ cnt: 0 })),
    db
      .prepare('SELECT type, COUNT(*) as cnt FROM asm_assets GROUP BY type ORDER BY cnt DESC')
