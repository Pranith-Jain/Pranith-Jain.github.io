...
const DOMAIN_RE = /(?:^|[^A-Za-z0-9@.\-_])((?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,24})(?![A-Za-z0-9-])/g
...
const REGISTRY_RE = /\b(?:HKEY_(?:LOCAL_MACHINE|CURRENT_USER|CLASSES_ROOT|CURRENT_CONFIG|USERS)|HK(?:LM|CU|CR|U|CC))[\\/][^\s"'<>]{2,}/gi
...
export function extractObservables(
  text: string,
  opts?: ExtractObservablesOptions
): ExtractObservablesResult {
  ...
  const overlaps = (cand: Candidate): boolean =>
    accepted.some((a) => cand.start < a.end && a.start < cand.end)
