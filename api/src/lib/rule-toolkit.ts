 ...
 */
function walkYaraCode(
  source: string,
  visit: (ch: string, idx: number) => void
): void {
  ...
  while (i < n) {
    ...
    if (mode === 'code') {
      if (ch === '/' && next === '/') { mode = 'line_comment'; i += 2; continue; }
      if (ch === '/' && next === '*') { mode = 'block_comment'; i += 2; continue; }
...
interface YaraSectionPos { meta?: number; strings?: number; condition?: number }
...
function validateYaraHexStrings(body: string, baseLine: number, errors: LineIssue[]): void {
  ...
  while ((m = HEX_RE.exec(body)) !== null) {
    const tokens = m[1]!.trim().split(/\s+/).filter((t) => t !== '' && t !== '-' && t !== '|');
...
export function validateYaraRule(source: string): YaraValidationResult {
  ...
  for (const b of bodies) {
    ...
    // slice section texts
    const ordered: Array<[keyof YaraSectionPos, number]> = (
      ['meta', 'strings', 'condition'] as const
    )
...
const SIGMA_COND_KEYWORDS = new Set(['and', 'or', 'not', 'of', 'them', 'all', 'any', '1', '2', '3', '4', '5', '6', '7', '8', '9'])
...
export function validateSigmaRule(yamlSource: string): SigmaValidationResult {
  ...
  else if (typeof id !== 'string' || !UUID_RE.test(id)) warnings.push({ message: `id "${String(id).slice(0, 40)}" is not a UUID` })
  const status = doc['status']
  if (status === undefined) warnings.push({ message: 'missing status (recommended: stable|test|experimental|deprecated)' })
  else if (typeof status !== 'string' || !SIGMA_STATUSES.has(status.toLowerCase())) warnings.push({ message: `unknown status "${String(status)}"` })
  const level = doc['level']
  if (level === undefined) warnings.push({ message: 'missing level (recommended: informational|low|medium|high|critical)' })
  else if (typeof level !== 'string' || !SIGMA_LEVELS.has(level.toLowerCase())) warnings.push({ message: `unknown level "${String(level)}"` })
  ...
  if (!isRecord(detection)) {
    ...
    else if (typeof condRaw !== 'string' && !Array.isArray(condRaw)) errors.push({ message: 'detection.condition must be a string or array of strings' })
    const selections = Object.keys(detection).filter((k) => k !== 'condition' && k !== 'timeframe')
    if (selections.length === 0) errors.push({ message: 'detection: no selections defined (need at least one named selection besides condition)' })
    else {
      ...
      const conds: string[] = typeof condRaw === 'string' ? [condRaw] : Array.isArray(condRaw) ? condRaw.filter((x): x is string => typeof x === 'string') : []
      for (const cond of conds) {
        ...
        for (const tok of toks) {
          ...
          if (tok.endsWith('*')) {
            ...
            if (!selections.some((s) => s.startsWith(prefix))) warnings.push({ message: `condition references "${tok}" which matches no selection` })
            continue
          }
  ...
  const summary: SigmaSummary | undefined = typeof title === 'string' && title.trim()
    ? { title: title.trim(), id: typeof id === 'string' ? id : undefined, level: typeof level === 'string' ? level : undefined, status: typeof status === 'string' ? status : undefined, tags: tagList, logsource: isRecord(ls) ? ls : {} }
    : undefined
...
function sigmaConditionTokens(cond: string): string[] {
  ...
  while (i < n) {
    ...
    if (ch === '"' || ch === "'") {
      const quote = ch
      let j = i + 1
      while (j < n && cond[j] !== quote) { if (cond[j] === '\\') j++; j++; }
...
type FieldOp = 'plain' | 'contains' | 'startswith' | 'endswith' | 're' | 'null' | 'gt' | 'gte' | 'lt' | 'lte' | 'unknown'
...
function parseFieldSpec(spec: string): { field: string; op: FieldOp; modifier?: string } {
  ...
  if (mod === 're' || mod === 're|contains' || mod === 'contains|windash' || mod === 'windash') return { field, op: 're', modifier: mod }
...
function fieldExprSplunk(field: string, op: FieldOp, raw: unknown, mappedField: string, warnings: string[]): string | null {
  ...
  if (op === 're') { warnings.push(`field ${field}: |re treated as plain quoted match in SPL (native regex not emitted)`); return `${mappedField}="${esc}"` }
...
function fieldExprKql(field: string, op: FieldOp, raw: unknown, mappedField: string, warnings: string[]): string | null {
...
function selectionToExpr(
  ...
  if (Array.isArray(selValue)) {
    ...
    if (allMaps) {
      const groups = (selValue as Record<string, unknown>[]).map((m) => {
        const parts = Object.entries(m).map(([k, v]) => {
          ...
          const exprs = vals.map((one) => fieldExpr(field, op, one, mapped, warnings)).filter((x): x is string => !!x)
  ...
  if (typeof selValue === 'object' && selValue !== null) {
    const parts = Object.entries(selValue as Record<string, unknown>).map(([k, v]) => {
      ...
      const mapped = fieldMap?.[field] ?? field
      const vals: unknown[] = Array.isArray(v) ? v as unknown[] : [v]
      const exprs = vals.map((one) => fieldExpr(field, op === 'unknown' ? 'plain' : op, one, mapped, warnings)).filter((x): x is string => !!x)
      return exprs.length === 1 ? exprs[0]! : `(${exprs.join(' OR ')})`
    }).filter(Boolean)
...
function expandSigmaCondition(
  cond: string,
  selectionExprs: Record<string, string>,
  warnings: string[]
): string {
...
function buildSigmaQuery(
  ...
  let doc: unknown
  try { doc = load(yamlSource) } catch (e) { return { ok: false, query: '', target, warnings, error: `YAML parse error: ${e instanceof Error ? e.message : String(e)}` } }
  ...
  const conds: string[] = typeof condRaw === 'string' ? [condRaw] : Array.isArray(condRaw) ? (condRaw as unknown[]).filter((x): x is string => typeof x === 'string') : []
  let body = ''
  if (conds.length === 0) {
  ...
  if (typeof doc['title'] === 'string') headers.push(target === 'splunk' ? `# title: ${doc['title']}` : `// title: ${doc['title']}`)
  const ls = doc['logsource']
  if (isRecord(ls)) {
    const lsParts = Object.entries(ls).map(([k, v]) => `${k}=${String(v)}`).join(' ')
...
const KNOWN_OSQUERY_TABLES = new Set([
  'processes','process_events','process_open_sockets','process_memory_map','process_envs',
  'users','logged_in_users','user_events','user_ssh_keys','groups',
  'file','file_events','file_changes','hash','yara','yara_events',
  'socket_events','listening_ports','interface_addresses','interface_details','routes','arp_cache','dns_resolvers','etc_hosts',
  'registry','registry_events','patches','programs','services','scheduled_tasks','startup_items','drivers','device_drivers',
  'chrome_extensions','firefox_addons','browser_plugins','safari_extensions',
  'certificates','carves','curl','curl_certificate','os_version','osquery_info','osquery_flags','osquery_schedule','time','uptime','system_info','ec2_instance_metadata','azure_instance_metadata',
  'ntfs_acl_permissions','windows_crashes','windows_events','windows_update_history','autoexec','ie_extensions','shared_resources','shares','logical_drives','physical_disk_performance',
  'mounts','block_devices','disk_encryption','ca_certs','last','logged_in_users','sudoers','authorized_keys',
  'package_bom','homebrew_packages','alf','alf_exceptions','crashes','launchd','launchd_overrides','managed_policies','gatekeeper','gatekeeper_approved_apps','sip_config','nvram',
  'kernel_modules','kernel_info','kernel_extensions','memory_map','memory_info','memory_array_mapped_addresses',
  'open_files','mdfind','spotlight','time_machine_backups','wifi_networks','wifi_scan','wifi_status','battery','temperature_sensors','usb_devices','connected_displays','apps','app_schemes','alf_exceptions','apparmor_profiles',
])
const HIGH_VOLUME_TABLES = new Set(['process_events','socket_events','file_events','yara_events','registry_events','user_events','windows_events'])
...
export function validateOsquerySql(sql: string): OsqueryValidationResult {
  ...
  if (semi >= 0 && trimmed.slice(semi + 1).trim().length > 0) errors.push({ message: 'multiple statements detected (semicolon with trailing content)' })
  ...
  for (let i = 0; i < trimmed.length; i++) {
    const ch = trimmed[i]!
    if (ch === "'" && !inDq) { if (i > 0 && trimmed[i - 1] === '\\') continue; inSq = !inSq; continue }
    if (ch === '"' && !inSq) { if (i > 0 && trimmed[i - 1] === '\\') continue; inDq = !inDq; continue }
    if (inSq || inDq) continue
    if (ch === '(') depth++
    else if (ch === ')') { depth--; if (depth < 0) { errors.push({ message: 'unbalanced closing parenthesis' }); depth = 0 } }
  }
  ...
  while ((fm = fromRe.exec(trimmed)) !== null) {
    ...
    if (!seen.has(tbl)) { seen.add(tbl); tables.push(tbl) }
    if (!KNOWN_OSQUERY_TABLES.has(tbl)) warnings.push({ message: `unknown osquery table "${tbl}"` })
    if (HIGH_VOLUME_TABLES.has(tbl) && !/\bWHERE\b/i.test(trimmed)) warnings.push({ message: `table "${tbl}" is high-volume; add a WHERE clause to bound the scan` })
  }
  const firstWord = trimmed.replace(/^\(+/, '').trim().split(/\s+/)[0]!.toUpperCase()
  if (firstWord !== 'SELECT' && firstWord !== 'WITH') errors.push({ message: `osquery SQL must start with SELECT or WITH (got ${firstWord})` })
...
const VALID_PROTOCOLS = new Set(['tcp', 'udp', 'icmp', 'ip', 'http', 'tls', 'dns', 'ssh', 'ftp', 'smtp', 'smb', 'dcerpc', 'dhcp', 'krb5', 'nfs', 'tftp', 'any'])
const KNOWN_MODIFIERS = new Set([
  'http_uri', 'http_raw_uri', 'http_header', 'http_raw_header', 'http_cookie', 'http_user_agent', 'http_host', 'http_raw_host',
  'http_method', 'http_stat_code', 'http_stat_msg', 'http_client_body', 'http_server_body',
  'dns_query', 'dns.query', 'tls.sni', 'tls.cert_subject', 'tls.cert_issuer', 'tls.certs', 'tls.version',
  'ssh.protoversion', 'ssh.softwareversion', 'file_data', 'file.name', 'file.magic',
  'urilen', 'dsize', 'isdataat', 'byte_test', 'byte_jump', 'byte_extract', 'pcre', 'content', 'nocase', 'rawbytes',
])
...
export function validateSuricataRule(ruleText: string): SuricataValidationResult {
  ...
  if (headerParts.length < 7) {
    errors.push({ message: `header requires 7 fields (action proto src sport dir dst dport), got ${headerParts.length}: "${header}"` })
    return { valid: false, errors, warnings }
  }
  const [actionRaw, protoRaw, src, sport, dir, dst, dport] = headerParts as [string, string, string, string, string, string, string]
  const action = actionRaw.toLowerCase()
  const proto = protoRaw.toLowerCase()
  if (!VALID_ACTIONS.has(action)) errors.push({ message: `unknown action "${actionRaw}" (expected alert|drop|pass|reject)` })
  if (!VALID_PROTOCOLS.has(proto)) warnings.push({ message: `uncommon protocol "${protoRaw}"` })
  if (dir !== '->' && dir !== '<>' && dir !== '<-') errors.push({ message: `direction must be -> or <> (got "${dir}")` })
  ...
  if (opts) {
    ...
    if (!sidM) warnings.push({ message: 'options: missing sid' })
      ...
      if (sid < 1000000) warnings.push({ message: `sid ${sid} is below 1000000 (reserved range; use >=1000000 for local rules)` })
    }
    if (!revM) warnings.push({ message: 'options: missing rev' })
    // content modifier warnings
    const optTokens = opts.split(';').map((s) => s.trim()).filter(Boolean)
    for (const tok of optTokens) {
      const lower = tok.toLowerCase().trim()
      if (!lower || lower.startsWith('msg') || lower.startsWith('sid') || lower.startsWith('rev') || lower.startsWith('classtype') || lower.startsWith('reference') || lower.startsWith('metadata')) continue
      ...
      if (lower.startsWith('pcre') || lower.startsWith('byte_') || lower.startsWith('flow') || lower.startsWith('flags') || lower.startsWith('threshold') || lower.startsWith('detection_filter') || lower.startsWith('distance') || lower.startsWith('within') || lower.startsWith('depth') || lower.startsWith('offset') || lower.startsWith('isdataat') || lower.startsWith('dsize') || lower.startsWith('urilen')) continue
  ...
  const parsed: SuricataParsed | undefined = headerParts.length >= 7 ? {
    action: actionRaw,
    protocol: protoRaw,
    sid: (() => { const mm = opts.match(/\bsid\s*:\s*(\d+)/i); return mm ? parseInt(mm[1]!, 10) : undefined })(),
    rev: (() => { const mm = opts.match(/\brev\s*:\s*(\d+)/i); return mm ? parseInt(mm[1]!, 10) : undefined })(),
    msg: (() => { const mm = opts.match(/\bmsg\s*:\s*"([^"]*)"/i); return mm ? mm[1] : undefined })(),
  } : undefined
...
export function validateRule(kind: RuleKind, source: string): YaraValidationResult | SigmaValidationResult | SuricataValidationResult | OsqueryValidationResult {
  switch (kind) {
    case 'yara': return validateYaraRule(source)
    case 'sigma': return validateSigmaRule(source)
    case 'suricata':
    case 'snort': return validateSuricataRule(source)
    case 'osquery': return validateOsquerySql(source)
    default: return { valid: false, errors: [{ message: `unknown rule kind "${kind}"` }], warnings: [] } as SigmaValidationResult
  }
}
