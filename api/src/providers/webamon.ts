...
export const webamon: ProviderAdapter = async (indicator, _env, signal) => {
  ...
  try {
    ...
    const url = `${WEBAMON_SEARCH}?search=${encodeURIComponent(`domain.name:${indicator.value}`)}&results=domain.name,page_title,meta.risk_score,meta.script_count,meta.report_id,resolved_url,tag,date,fingerprint.tech,fingerprint.asn,fingerprint.ssl&size=1`;
