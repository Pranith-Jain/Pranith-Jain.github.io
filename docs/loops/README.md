...
| [Canary After Deploy](canary-after-deploy.md)               | After `wrangler deploy` returns                    | Public pages 200 + MCP auth-gate 401 + `smoke` green                                    |
| [Social Crosspost](social-crosspost.md)                     | Publishing a case-study/blog asset to X + LinkedIn | Both platforms `status: posted` with `postUrl` captured                                 |
