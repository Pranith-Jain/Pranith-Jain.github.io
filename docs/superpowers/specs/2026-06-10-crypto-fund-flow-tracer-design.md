...
| **B — Labeling & attribution depth**  | D1 `address_labels` + user-add; BTC common-input-ownership clustering heuristic; `/tracer/paths` auto-path to CEX/Mixer; optional `threat-graph` cross-upsert.                                                               | new clustering; reuse `shortestPath`                                   |
