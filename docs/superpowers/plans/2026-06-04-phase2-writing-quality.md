...
> **⚠️ SCOPE CHANGE (2026-06-04): SKIP the model-swap tasks.** The premise that Groq `llama-4-scout-17b` is deprecated was wrong (re-verified: it's not on Groq's shutdown list). The user decided to **keep Llama-4-Scout — do NOT touch `ai-client.ts`.** Therefore **skip Task 1 (model swap) and Task 2 (model-threading + no-chain-of-thought guardrail)** entirely. Execute only: **Task 3** (AEO/GEO blog prompt in `templates.ts`), **Task 4** (structural QA in `post-process.ts`), **Task 5–6** (JSON-LD schema + visible "Updated" date + AI-crawler robots.txt), **Task 7** (regression). The `reasoning_effort` / gpt-oss guardrail bits in Task 3+ are not needed; ignore any reference to them.

**Goal (revised):** Rewrite the blog prompt to 2026 AEO/GEO norms (TL;DR + query-shaped H2s + FAQ + named detections + IOC table + estimative-language discipline + stat density), enforce those elements in structural QA, and emit `BlogPosting`+`FAQPage` JSON-LD plus a visible "Updated" date and AI-crawler allow rules on the render layer. **No model change** (Llama-4-Scout stays).
