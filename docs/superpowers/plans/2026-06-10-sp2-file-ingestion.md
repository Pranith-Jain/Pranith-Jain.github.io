...
> **Running tests (read first):** This repo's `api/` tests run under
> `@cloudflare/vitest-pool-workers`, which the harness sandbox blocks. Run every
> `npx vitest run ...` command by invoking the **Bash tool with
> `dangerouslyDisableSandbox: true`** — this is a Bash-tool parameter, NOT a
> vitest CLI flag (`npx vitest run --dangerouslyDisableSandbox` errors). All
> `npx vitest run` commands below assume the Bash sandbox is disabled this way.
...
Run: `cd api && npx vitest run test/routes/report-ingest-middleware.test.ts` (Bash tool: `dangerouslyDisableSandbox: true`)
Expected: PASS (2 tests). (Route tests need the harness sandbox disabled — repo footgun.)
...
Run: `cd api && npx vitest run test/lib/file2txt/ test/routes/report-ingest.test.ts test/routes/report-ingest-middleware.test.ts test/routes/intel-bundle.test.ts` (Bash tool: `dangerouslyDisableSandbox: true`)
Expected: all PASS.
