...
function slideColors(spec: ContentSpec, slide: ContentSlide): { bg: string; fg: string; accent: string } {
  ...
  // Hook slides use funnel tint background for visual distinction
  // CTA slides use solid accent color
  // All other slides use white
  const defaultBg = isHook ? funnel.bg : (slide.isCTA ? funnel.accent : BRAND.colors.neutral.white);
  const bg = slide.bg ?? defaultBg;
...
function renderSlide(spec: ContentSpec, slide: ContentSlide, slideWidth: number, slideHeight: number): string {
  ...
  const funnelBadgeBg = isCTA ? 'rgba(255,255,255,0.2)' : `${accent}15`;
  const funnelBadgeBorder = isCTA ? 'rgba(255,255,255,0.3)' : `${accent}30`;
  const funnelBadge = `
    <div style="position:absolute; top:24px; right:24px; font-size:11px; font-family:${BRAND.fonts.mono};
      ...
      background:${funnelBadgeBg}; color:${isCTA ? fg : accent};
      border:1px solid ${funnelBadgeBorder};">
