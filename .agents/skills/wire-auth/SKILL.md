...
export async function POST(request: Request) {
  ...
  const { data: existing } = await supabase.from('users').select('id, auth_id').eq('auth_id', userId).maybeSingle();
  ...
  if (!existing) {
    ...
    if (byEmail) {
      ...
      await supabase.from('users').update({ auth_id: userId, name, avatar_url: imageUrl }).eq('id', byEmail.id);
    } else {
      // Create new user
  ...
  const { data: user } = await supabase.from('users').select('*').eq('auth_id', userId).single();
