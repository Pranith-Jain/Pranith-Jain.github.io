...
export async function sendWaitlistConfirmation({
  ...
  const referralUrl = referralCode ? `${process.env.NEXT_PUBLIC_APP_URL}/?ref=${referralCode}` : null;
  ...
  if (state === 'success') {
    return (
      <div>
        ...
        {userReferralCode && (
          <p>
            share your link to move up: <code>{`${window.location.origin}/?ref=${userReferralCode}`}</code>
          </p>
        )}
      </div>
    );
